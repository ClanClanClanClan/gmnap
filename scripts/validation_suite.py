...
def run_validation_suite():
    ...
    print(f"  Conversion success rate: {conversion_rate:.1f}% ({conversion_successes}/{total_tests})")
    print(f"  Round-trip success rate: {round_trip_rate:.1f}% ({round_trip_successes}/{total_tests})")
    ...
    if failed_cases:
        ...
        for i, case in enumerate(failed_cases[:10]):
            print(f"  {i+1:2d}. {case['original']} -> {case['hangul']} -> {case['back_converted']} (Dice: {case['dice_score']:.3f})")
    
    # Overall pass/fail
    overall_pass = (conversion_rate >= blueprint_conversion_target and 
                   round_trip_rate >= blueprint_roundtrip_target and 
                   avg_dice >= 0.85)
