...
class TestReportGenerator:
    ...
    def run_tests_with_reporting(self):
        ...
        test_commands = [
            {
                "name": "unit_tests",
                "cmd": ["python", "-m", "pytest", "tests/unit/", "-v", "--junitxml=test-results-unit.xml", "--cov=src", "--cov-report=xml:coverage-unit.xml"],
                "description": "Unit Tests"
            },
            {
                "name": "property_tests", 
                "cmd": ["python", "-m", "pytest", "tests/property/", "-v", "--junitxml=test-results-property.xml", "-m", "property"],
                "description": "Property-Based Tests"
            },
            {
                "name": "security_tests",
                "cmd": ["python", "-m", "pytest", "tests/security/", "-v", "--junitxml=test-results-security.xml", "-m", "security"],
                "description": "Security Tests"
            },
            {
                "name": "memory_tests",
                "cmd": ["python", "-m", "pytest", "tests/memory/", "-v", "--junitxml=test-results-memory.xml", "-m", "memory", "--tb=short"],
        ...
        for test_config in test_commands:
            ...
            try:
                ...
                print(f"  {test_config['description']}: {'PASSED' if result.returncode == 0 else 'FAILED'}")
    ...
    def analyze_performance_data(self) -> Dict[str, Any]:
        ...
        if "testcases" in memory_xml:
            ...
            if test_times:
                ...
                slowest = sorted(memory_xml["testcases"], key=lambda x: x["time"], reverse=True)[:10]
                performance_data["slowest_tests"] = [
                    {"name": test["name"], "time": test["time"]} 
                    for test in slowest if test["time"] > 0
                ]
            ...
            if result.returncode == 0 or result.stdout:
                try:
                    ...
                    security_data["bandit_results"] = {
                        ...
                        "confidence_high": sum(1 for r in bandit_data.get("results", []) if r.get("issue_confidence") == "HIGH"),
                        "severity_high": sum(1 for r in bandit_data.get("results", []) if r.get("issue_severity") == "HIGH"),
                        "severity_medium": sum(1 for r in bandit_data.get("results", []) if r.get("issue_severity") == "MEDIUM")
                    }
            ...
            if result.stdout:
                try:
                    ...
                    security_data["safety_results"] = {
                        ...
                        "high_severity": sum(1 for v in safety_data if v.get("advisory", {}).get("severity") == "high")
                    }
                except json.JSONDecodeError:
...
<body>
    ...
    <div class="details">
        ...
        overall_status = "success" if success_percentage >= 90 else "warning" if success_percentage >= 70 else "danger"
        ...
        html_content = html_template.format(
            ...
            security_status="success" if self.report_data["security"].get("total_issues", 0) == 0 else "danger",
...
def main():
    ...
    try:
        ...
        print(f"\n✅ Test report successfully generated: {report_file}")
        ...
        if summary.get("total_tests", 0) > 0:
            ...
            if summary['failed_tests'] > 0:
                print(f"\n⚠️  {summary['failed_tests']} tests failed. Please review the detailed report.")
                sys.exit(1)
            else:
                print(f"\n🎉 All tests passed!")
