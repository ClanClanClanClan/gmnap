...
def main():
    ...
    fix_map = {fix['romanization']: fix['hangul'] for fix in high_conf_fixes if fix['type'] == 'add_mapping'}
    ...
    if conflicts > 0:
        ...
        for ex in conflict_examples:
            print(f"  - {ex['name']}: {ex['current']} → {ex['would_change_to']} (due to {ex['fix']})")
    else:
    ...
    report = {
        'summary': {
            ...
            'recommendation': 'APPLY' if reward_score > risk_score * 2 else 'REVIEW' if reward_score > risk_score else 'REJECT'
        },
        'top_10_fixes': [
            {
