...
# Get recent commits
all_shas = run("git rev-list --max-count=10 HEAD", cwd=REPO_ROOT, quiet=False).stdout.strip().splitlines()
...
for i, sha in enumerate(all_shas):
    ...
    # Run diverse test
    r2 = run(f"cd {KOREAN_DIR} && python3 test_accuracy.py data/korean_diverse_test.yaml", quiet=True)
    if r2.returncode:
    ...
    try:
        ...
        print(f"  Math: {m_ok}/{m_tot} ({100*m_ok/m_tot:.1f}%), Diverse: {d_ok}/{d_tot if d_tot else 0} ({100*d_ok/d_tot if d_tot else 0:.1f}%)")
        print(f"  Message: {msg}")
