...
DIVERSE_SCRIPT = REPO_ROOT / "src" / "gmnap" / "regions" / "e_groups" / "e4_korea" / "test_accuracy.py"
BUILD_SCRIPT = REPO_ROOT / "src" / "gmnap" / "regions" / "e_groups" / "e4_korea" / "scripts" / "build_fsts_multi.py"
RESULTS = {}
...
all_shas = (
    run("git rev-list --max-count=200 --reverse HEAD", cwd=REPO_ROOT, quiet=False).stdout.strip().splitlines()
)
...
with TemporaryDirectory() as tmp:
    ...
    for sha in tqdm(all_shas, desc="Scanning commits"):
        ...
        korean_data = REPO_ROOT / "src" / "gmnap" / "regions" / "e_groups" / "e4_korea" / "data" / "korean.yaml"
        diverse_data = REPO_ROOT / "src" / "gmnap" / "regions" / "e_groups" / "e4_korea" / "data" / "korean_diverse_test.yaml"
