...
class ArchitectureAwareAnalyzer:
    ...
    def analyze_failure(self, name: str, expected: str, actual: Optional[str], 
                       is_surname: bool = False) -> Optional[Fix]:
        ...
        if actual is None:
            ...
            fix_type, viable = self.classify_fix(pattern, expected, 
                                                'surname' if is_surname else 'given')
            ...
            if viable:
                ...
                if fix_type == FixType.MULTI_SYLLABLE:
                    impl = (f'# WARNING: "{pattern}" segments to {self.test_segmentation(pattern)}\n'
        ...
        for failure in failures:
            ...
            if fail_type == 'eng→kor':
                ...
                if len(parts) >= 2:
                    ...
                    if actual is None or (actual and len(actual) > 0 and len(expected) > 0 and actual[0] != expected[0]):
                        print(f"    Surname issue detected: {surname} -> {expected[0] if expected else 'N/A'}")
                        fix = self.analyzer.analyze_failure(
                            surname, expected[0] if expected else '', 
                            actual[0] if actual and len(actual) > 0 else None, is_surname=True
                        )
                        if fix:
                            fix_patterns[f"surname_{surname}"].append(fix)
                            print(f"      Generated fix: {fix.pattern} -> {fix.hangul} (viable: {fix.viable})")
...
def main():
    ...
    print(f"1. Apply {viable_count} viable fixes immediately (est. +{viable_count * 0.5}% accuracy)")
