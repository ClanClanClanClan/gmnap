...
\]'''
    ...
    if match:
        ...
        items = [item.strip().strip("'\"") for line in list_content.split('\n') 
                for item in line.split(',') if item.strip().strip("'\"")]
        ...
        # Replace in content
        replacement = "        # Czech/Polish surname patterns (fix for G1→B2 misclassification)\n" + "\n".join(lines)
