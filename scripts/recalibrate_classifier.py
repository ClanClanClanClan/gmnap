...
def recalibrate_name_classifier():
    ...
    classifier_params = {
        ...
        'feature_names': ['length', 'word_count', 'hyphen_count', 'capital_count', 'common_surname'],
    ...
    for name in test_names:
        features = np.array([[
            ...
            1 if any(c.lower() in name.lower() for c in ['kim', 'lee', 'park', 'choi']) else 0
        ]])
        ...
        print(f"  {status} {name:15s}: {prob:.1%} confidence {'(PASS)' if actual_result else '(FAIL)'}")
