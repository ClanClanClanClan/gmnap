...
def benchmark(sizes: list[int], real_names: bool = False, profile: bool = False):
    ...
    ``profile``: when True, wrap the pipeline run in cProfile and dump
    cumulative-time stats to ``docs/perf_profile_<size>_<mode>.txt``.
    Used to drive the in-process-fastText decision (round 26 deferred
    that on the hypothesis fastText isn't the bottleneck — profile
    confirms or rejects).
    """
    label = "REAL names" if real_names else "synthetic"
    print(f"Mode: {label}{' [PROFILE]' if profile else ''}")
    print()
    print(
    ...
    for n in sizes:
        ...
        try:
            if profile:
                ...
                mode_label = "real" if real_names else "syn"
                out_path = Path(f"docs/perf_profile_{n}_{mode_label}.txt")
                out_path.parent.mkdir(parents=True, exist_ok=True)
                with out_path.open("w", encoding="utf-8") as f:
                    f.write(
                        f"# cProfile dump — n={n} mode={mode_label}\n"
                        f"# Generated by tools/run_benchmark.py --profile\n\n"
                    )
                    # Top 60 by cumulative time
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Wrap each pipeline run in cProfile; dump cumulative-time "
            "stats to docs/perf_profile_<size>_<mode>.txt. Used for "
            "bottleneck analysis to decide whether to optimize "
            "specific stages (round-28 introduced this for the "
            "in-process-fastText decision)."
        ),
    )
    ...
    print(f"Sizes: {sizes}")
    benchmark(sizes, real_names=args.real_names, profile=args.profile)
