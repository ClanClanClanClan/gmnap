...
def analyze_directory_structure():
    ...
    duplicates = {
        "src/core vs src/gmnap/core": {
            "old": list(Path(root / "src/core").glob("*.py")) if (root / "src/core").exists() else [],
            "new": list(Path(root / "src/gmnap/core").glob("*.py")) if (root / "src/gmnap/core").exists() else []
        },
        "src/authorities vs src/gmnap/authorities": {
            "old": list(Path(root / "src/authorities").glob("**/*.py")) if (root / "src/authorities").exists() else [],
            "new": list(Path(root / "src/gmnap/authorities").glob("**/*.py")) if (root / "src/gmnap/authorities").exists() else []
        }
    }
    ...
    scattered_tests = {
        "cleanup_work/test_scripts": len(list(Path(root / "cleanup_work/test_scripts").glob("*.py"))) if (root / "cleanup_work/test_scripts").exists() else 0,
        "korean tests": len(list(Path(root / "src/gmnap/regions/e_groups/e4_korea/tests").glob("*.py"))) if (root / "src/gmnap/regions/e_groups/e4_korea/tests").exists() else 0
    }
...
def generate_move_commands(report):
    ...
    if any(report["scattered_tests"].values()):
        ...
        if report["scattered_tests"]["korean tests"] > 0:
            ...
            commands.append("mv src/gmnap/regions/e_groups/e4_korea/tests/* tests/unit/korean/ 2>/dev/null || true")
        if report["scattered_tests"]["cleanup_work/test_scripts"] > 0:
            commands.append("mv cleanup_work/test_scripts/test_*.py tests/integration/ 2>/dev/null || true")
            commands.append("mv cleanup_work/test_scripts/comprehensive_*.py tests/quality_gates/ 2>/dev/null || true")
        commands.append("")
