...
class PipelinePatternIntegratorFixed:
    def __init__(self, pipeline_path="/Users/dylanpossamai/Library/CloudStorage/Dropbox/Work/Maths/gmnap/src/gmnap/core/pipeline.py"):
    ...
    def integrate_hungarian_patterns(self, content, hungarian_patterns):
        ...
        if match:
            ...
            current_list = [p.strip().strip("'\"") for p in current_patterns.split(',') if p.strip()]
            
            # Add YAML patterns (remove accents for matching, limit to most important)
            yaml_patterns = []
            priority_patterns = ['erdős', 'erdos', 'rényi', 'renyi', 'kőnig', 'konig', 'lovász', 'lovasz',
                               'turán', 'turan', 'fejér', 'fejer', 'bolyai', 'császár', 'csaszar']
            
            for pattern in hungarian_patterns:
                # Remove accents for pattern matching
                clean = pattern.lower()
                clean = clean.replace('ő', 'o').replace('ű', 'u').replace('á', 'a')
                clean = clean.replace('é', 'e').replace('í', 'i').replace('ó', 'o').replace('ú', 'u')
                
                # Only add if it's in priority list or not already present
                if (clean in priority_patterns or len(yaml_patterns) < 15) and clean not in current_list:
            ...
            print(f"   🎯 Key additions: {[p for p in yaml_patterns if any(target in p for target in ['erdos', 'renyi', 'fejer', 'turan'])]}")
    ...
    def integrate_korean_patterns(self, content, korean_patterns):
        ...
        if match:
            ...
            yaml_patterns = []
            priority_surnames = ['ahn', 'baek', 'choi', 'cho', 'jang', 'jeong', 'jung', 'kang', 'kim', 'lee', 'lim', 'moon',
                                'nam', 'oh', 'park', 'ryu', 'seo', 'shin', 'song', 'yang', 'yoon', 'han', 'hong', 'kwon']
            ...
            for pattern in korean_patterns:
                ...
                if (clean in priority_surnames or 
                    (len(clean) <= 4 and clean not in current_list and 
                     not any(char.isdigit() for char in clean) and
                     '-' not in clean)):  # Avoid hyphenated given names
    ...
    def integrate_slavic_patterns(self, content, slavic_patterns):
        ...
        hungarian_section = re.search(r'(if has_hungarian_surname:.*?scores\[\'G1\'\] = max\(0, scores\[\'G1\'\] - 4\))', content, re.DOTALL)
        
        if hungarian_section:
            # Create selective Slavic patterns (focus on mathematician surnames)
            priority_slavic = ['hájek', 'hajek', 'novák', 'novak', 'banach', 'sierpiński', 'sierpinski', 
                              'bartoszyński', 'bartoszynski', 'borsuk', 'kuratowski', 'tarski', 'łojasiewicz']
        ...
        if has_slavic_surname:
            ...
            print(f"   🎯 Key additions: {[p for p in filtered_patterns if any(target in p for target in ['hajek', 'novak', 'banach', 'sierpinski'])]}")
    ...
    def boost_korean_detection(self, content):
        ...
        korean_pattern_section = re.search(r'(has_korean_pattern = any\(name_lower\.startswith\(surname \+ \',\'\) or name_lower\.startswith\(surname \+ \' \'\) for surname in korean_surnames\))', content)
