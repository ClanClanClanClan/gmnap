...
class PipelinePatternIntegrator:
    ...
    def integrate_slavic_patterns(self, content, slavic_patterns):
        ...
        b2_section = re.search(r'# Slavic \(B1/B2\) detection.*?scores\[\'B2\'\] \+= \d+', content, re.DOTALL)
        ...
        if has_slavic_surname:
            ...
            '''
                ...
                print(f"   🎯 Key additions: {[p for p in slavic_patterns if any(target in p for target in ['hajek', 'novak', 'banach', 'sierpinski'])]}")
    ...
    def boost_korean_detection(self, content):
        ...
        korean_boost = re.search(r'(has_korean_pattern = any.*?korean_surnames\))', content, re.DOTALL)
