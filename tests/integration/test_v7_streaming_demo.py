...
def test_old_approach(entries_list, desc):
    ...
    for i, entry in enumerate(entries_list):
        ...
        if (i + 1) % 10000 == 0:
            ...
            print(f"  {i+1:,} entries: Memory {current_memory:.1f} MB (+{growth:.1f} MB)")
    ...
    print(f"  Final memory: {final_memory:.1f} MB (+{final_memory - initial_memory:.1f} MB)")
    print(f"  Memory per 1K: {(final_memory - initial_memory) / (len(entries_list) / 1000):.3f} MB")
...
def test_v7_streaming(input_file, num_entries, desc):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config = V7StreamConfig(chunk_size=8000, output_dir=output_dir, enable_compression=False)
...
def main():
    ...
    for num_entries in test_sizes:
        ...
        print(f"Created test file: {temp_file} ({temp_file.stat().st_size / (1024**2):.1f} MB)")
        ...
        # Test V7 streaming
        v7_memory = test_v7_streaming(temp_file, num_entries, f"{num_entries:,} entries")
