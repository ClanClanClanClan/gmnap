...
if memory_points:
    ...
    if current_leak < 0.01:
        ...
        print("   The leak is definitely in the object creation/retention")
    elif current_leak >= object_creation_leak * 0.8:
        print("\n🔴 PYTHON BASELINE LEAK: Memory growth even with minimal processing")
        print("   This suggests Python internal memory management or test artifacts")
    else:
    ...
    if current_leak < 0.01:
        ...
        print("   -> RegionDetectionResult dataclass or its metadata dictionary")
        print(
            "   -> Need to investigate dataclass field retention or circular references"
        )
        print("   -> Potential fix: Use slots, or avoid default_factory=dict")
    elif current_leak < 0.02:
        print("\n📈 LIKELY LEAK SOURCES:")
        print("   -> Python string interning or dictionary growth")
        print("   -> RegionDetectionResult object retention")
        print("   -> Need memory profiling with tracemalloc")
    else:
        print("\n📈 INVESTIGATION NEEDED:")
        print("   -> Memory leak may be in test setup or Python runtime")
        print("   -> Need proper memory profiling to identify source")
else:
