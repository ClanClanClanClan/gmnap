...
class TestPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_quick_mode_pipeline(self):
        ...
        with patch('src.authorities.tier0.openalex.OpenAlexFetcher') as mock_fetcher:
            ...
            result = await pipeline.run(self.input_dir)
    ...
    @pytest.mark.asyncio
    async def test_stage_sequencing(self):
        ...
        async def mock_stage(stage_name):
            execution_order.append(stage_name)
            return Mock()
        
        # Mock all stages
        with patch.object(pipeline, '_stage_0_config', side_effect=lambda: mock_stage("stage_0")):
            with patch.object(pipeline, '_stage_1_ingest', side_effect=lambda x: mock_stage("stage_1")):
                with patch.object(pipeline, '_stage_2_detect_region', side_effect=lambda: mock_stage("stage_2")):
                    with patch.object(pipeline, '_stage_3_region_hooks', side_effect=lambda: mock_stage("stage_3")):
                        with patch.object(pipeline, '_stage_4_authority_enrich', side_effect=lambda: mock_stage("stage_4")):
                            with patch.object(pipeline, '_stage_5_collision_analytics', side_effect=lambda: mock_stage("stage_5")):
                                with patch.object(pipeline, '_stage_6_tag_short_forms', side_effect=lambda: mock_stage("stage_6")):
                                    with patch.object(pipeline, '_stage_7_global_validate', side_effect=lambda: mock_stage("stage_7")):
                                        with patch.object(pipeline, '_stage_8_write_diff', side_effect=lambda: mock_stage("stage_8")):
                                            with patch.object(pipeline, '_stage_9_report', side_effect=lambda: mock_stage("stage_9")):
                                                with patch.object(pipeline, '_stage_10_idempotency_check', side_effect=lambda: mock_stage("stage_10")):
    ...
    @pytest.mark.asyncio
    async def test_region_detection_integration(self):
        """Test region detection with real entries."""
        self.create_mixed_region_entries()
        
        # Mock FastText to avoid loading model
        with patch('src.regions.manager_new.fasttext') as mock_fasttext:
            mock_model = Mock()
            ...
            await pipeline._stage_1_ingest(self.input_dir)
    ...
    @pytest.mark.asyncio
    async def test_region_hooks_integration(self):
        ...
        await pipeline._stage_1_ingest(self.input_dir)
        await pipeline._stage_2_detect_region()
        await pipeline._stage_3_region_hooks()
    ...
    @pytest.mark.asyncio
    async def test_authority_enrichment_integration(self):
        ...
        with patch('src.authorities.tier0.openalex.OpenAlexFetcher') as mock_fetcher:
            ...
            await pipeline._stage_2_detect_region()
            await pipeline._stage_3_region_hooks()
            await pipeline._stage_4_authority_enrich()
    ...
    @pytest.mark.asyncio
    async def test_collision_analytics_integration(self):
        ...
        await pipeline._stage_3_region_hooks()
        await pipeline._stage_4_authority_enrich()
        await pipeline._stage_5_collision_analytics()
    ...
    async def test_validation_integration(self):
        ...
        await pipeline._stage_4_authority_enrich()
        await pipeline._stage_5_collision_analytics()
        await pipeline._stage_6_tag_short_forms()
        await pipeline._stage_7_global_validate()
        
        # Check that validation errors were recorded
        assert len(pipeline._metrics.validation_errors) > 0
        assert any("GlobalID" in error for error in pipeline._metrics.validation_errors)
    ...
    async def test_yaml_roundtrip(self):
        ...
        await pipeline._stage_6_tag_short_forms()
        await pipeline._stage_7_global_validate()
        await pipeline._stage_8_write_diff()
    ...
    async def test_error_handling(self):
        ...
        with patch.object(pipeline, '_stage_4_authority_enrich', side_effect=Exception("Test error")):
            ...
            # Pipeline should handle error and continue
            assert result.mode == PipelineMode.QUICK
            assert len(result.stage_metrics) > 0
            
            # Check error was recorded
            stage_4_metrics = result.stage_metrics.get("stage_4")
            if stage_4_metrics:
                assert len(stage_4_metrics.errors) > 0
