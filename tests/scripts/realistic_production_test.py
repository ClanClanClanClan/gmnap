...
class RealisticProductionTest:
    ...
    def test_realistic_scale_performance(self, sizes: List[int]) -> Dict[str, Any]:
        ...
        for size in sizes:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if success:
                    print(
                        f"  PASS Success: {duration:.1f}s ({entries_per_second:.2f} entries/sec)"
                    )
    ...
    def _generate_final_assessment(self, results: Dict[str, Any]):
        ...
        if max_successful_size >= 500:
            ...
            print(
                f"WARN Limit to demonstration/testing use (<{max_successful_size} entries)"
            )

        if best_performance > 30:
            print(
                f"WARN Expect {best_performance/30:.1f}x longer processing than target"
            )

        if coverage_percentage < 80:
            missing_percentage = 100 - coverage_percentage
            print(
                f"WARN {missing_percentage:.1f}% of global mathematicians not covered"
            )
