...
class PerformanceProfiler:
    ...
    def assert_performance(self, operation_name: str, max_time: float = None, max_memory_mb: float = None):
        ...
        if max_time:
            ...
            assert actual_time <= max_time, f"Operation {operation_name} took {actual_time:.3f}s, expected <= {max_time}s"
        
        if max_memory_mb:
            memory_delta_mb = measurement["memory_delta"] / 1024 / 1024
            assert memory_delta_mb <= max_memory_mb, f"Operation {operation_name} used {memory_delta_mb:.2f}MB, expected <= {max_memory_mb}MB"
    ...
    def test_large_entry_validation(self, profiler):
        ...
        large_entry = {
            "García, Juan Carlos": {
                ...
                "LanguageOfPublication": ["en", "es", "fr", "de", "it", "pt", "ca", "eu", "gl", "an"],
                "AffiliationTimeline": [
                ...
                "Variants": {
                    "Observed": [
                        {"str": f"García{i}, J. C.", "source": "MathSciNet", "accessed": "2025-07-15"}
                        for i in range(100)
                    ],
                    "Synthesised": [
                ...
                "PrimaryMSC": [
                    ...
                    for i in range(10, 20) for j in range(10, 15)
                ],
                "NameEvents": [
                    {"type": "marriage", "year": 2005, "from": "Juan Carlos García", "to": "Juan Carlos García Marín"},
                    {"type": "passport", "year": 2010, "from": "Juan Carlos García Marín", "to": "Juan Carlos García-Marín"},
                ],
