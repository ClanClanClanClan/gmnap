...
def name_strategy(draw):
    ...
    family_name = draw(text(
        alphabet=characters(
            min_codepoint=0x0020, max_codepoint=0x017F,  # Basic Latin + Latin-1
            categories=["Lu", "Ll", "Pd", "Po"]
        ),
        min_size=1, max_size=30
    ))
    
    given_name = draw(text(
        alphabet=characters(
            min_codepoint=0x0020, max_codepoint=0x017F,
            categories=["Lu", "Ll", "Pd", "Po"]
        ),
        min_size=1, max_size=30
    ))
...
def combining_text_strategy(draw):
    ...
    combining_chars = draw(lists(
        characters(categories=['Mn', 'Mc']),  # Combining marks
...
class TestUnicodeNormalizationProperties:
    ...
    def test_name_normalization_properties(self, name):
        ...
        assert normalized.count(", ") == name.count(", "), f"Comma count changed: {name} -> {normalized}"
        
        # Should preserve word boundaries
        name_words = name.split()
        normalized_words = normalized.split()
        assert len(name_words) == len(normalized_words), f"Word count changed: {name} -> {normalized}"
...
class TestUnicodeNormalizationRegression:
    ...
    def test_specific_ligature_cases(self):
        ...
        for input_text, expected in test_cases:
            ...
            assert result == expected, f"Ligature test failed: {input_text} -> {result}, expected {expected}"
