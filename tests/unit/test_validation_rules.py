...
class TestSlavicPatronymic:
    def test_valid_patronymic_passes(self, engine):
        ...
        patronymic_results = [r for r in results if r.rule_id == "b1_slavic_patronymic_check"]
    ...
    def test_short_patronymic_warns(self, engine):
        ...
        if patronymic_results:
            assert any("Short patronymic" in w for r in patronymic_results for w in r.warnings)
...
class TestCentralAsianScript:
    def test_post_2000_cyrillic_warns(self, engine):
        ...
        reform_results = [r for r in results if r.rule_id == "c1_central_asian_script_reform"]
        if reform_results:
            assert any("reform" in w.lower() for r in reform_results for w in r.warnings)
...
class TestArabicArticle:
    def test_consistent_article_ok(self, engine):
        ...
        article_results = [r for r in results if r.rule_id == "c5_arabic_definite_article"]
...
class TestDravidianInitial:
    def test_bare_initial_warns(self, engine):
        ...
        initial_results = [r for r in results if r.rule_id == "d2_dravidian_initial_prefix"]
        if initial_results:
            assert any("initial" in w.lower() for r in initial_results for w in r.warnings)
...
class TestFrancophoneParticle:
    def test_particle_present(self, engine):
        ...
        particle_results = [r for r in results if r.rule_id == "f1_francophone_particle"]
...
class TestIndigenousNaming:
    def test_mononym_with_comma_warns(self, engine):
        ...
        if indigenous_results:
            assert any("comma" in w.lower() for r in indigenous_results for w in r.warnings)
