...
from src.core.pipeline import (DataIngestionStage, Pipeline, PipelineConfig,
                               PipelineMode, PipelineStage, StageResult,
                               StageStatus, ValidationStage)
...
class TestPipelineIntegration:
    ...
    async def test_full_pipeline_flow(self):
        ...
        assert result["status"] == "completed"
        assert len(result["stages"]) == 2
        assert result["metrics"]["total_records_processed"] == 20  # 10 from ingestion + 10 from validation
