...
from src.core.unicode_handler import (UnicodeConfig, UnicodeNormalizer,
                                      generate_name_variants, normalize_name)
...
class TestUnicodeNormalizationSecurity:
    ...
    def test_homograph_attack_detection(self):
        ...
        for latin_name, cyrillic_name in homograph_pairs:
            ...
            assert self.normalizer.is_mixed_script(cyrillic_name, threshold=0.05), f"Failed to detect mixed script in: {cyrillic_name}"
            ...
            assert 'Cyrillic' not in latin_scripts, f"Latin text should not contain Cyrillic: {latin_name}"
            
            # Cyrillic version should contain both scripts
            assert 'Cyrillic' in cyrillic_scripts, f"Cyrillic text should contain Cyrillic script: {cyrillic_name}"
            assert 'Latin' in cyrillic_scripts, f"Mixed text should contain Latin script: {cyrillic_name}"
    ...
    def test_unicode_normalization_attacks(self):
        ...
        for attack_vector in attack_vectors:
            ...
            assert len(normalized) <= len(attack_vector) * 2, f"Normalization expanded too much: {repr(attack_vector)}"
    ...
    def test_script_confusion_attacks(self):
        ...
        for name in confusing_names:
            ...
            assert self.normalizer.is_mixed_script(name), f"Failed to detect mixed script in: {name}"
    ...
    def test_unicode_bidi_attacks(self):
        ...
        for attack in bidi_attacks:
            ...
            for char in dangerous_bidi:
                assert char not in normalized, f"Dangerous bidi character preserved: {repr(char)} in {repr(attack)}"
    ...
    def test_unicode_length_attacks(self):
        ...
        for attack in length_attacks:
            ...
            assert growth_ratio <= 2.0, f"Excessive length growth: {growth_ratio:.2f}x for {len(attack)} chars"
            ...
            assert normalization_time < 1.0, f"Normalization too slow: {normalization_time:.2f}s for {len(attack)} chars"
...
class TestUnicodePerformance:
    ...
    def test_normalization_performance(self):
        ...
        for test_string in test_strings:
            ...
            chars_per_second = len(test_string) / normalization_time if normalization_time > 0 else float('inf')
            assert chars_per_second > 10000, f"Normalization too slow: {chars_per_second:.0f} chars/sec"
    ...
    def test_script_detection_performance(self):
        ...
        for script_name, test_string in test_cases:
            ...
            assert detection_time < 0.1, f"Script detection too slow: {detection_time:.3f}s for {script_name}"
            
            # Should detect reasonable script
            assert detected_script in ['Latin', 'Cyrillic', 'Arabic', 'CJK', 'Kana', 'Other'], \
    ...
    def test_concurrent_normalization(self):
        ...
        # Verify results
        assert len(worker_results) == num_workers, f"Not all workers completed: {len(worker_results)}"
...
class TestUnicodeEdgeCases:
    ...
    def test_single_character_edge_cases(self):
        ...
        for char in high_surrogates:
            try:
                normalized = self.normalizer.normalize(char)
                assert isinstance(normalized, str), f"Failed to normalize high surrogate: {repr(char)}"
            except UnicodeError:
        ...
        for char in private_use:
            normalized = self.normalizer.normalize(char)
            assert isinstance(normalized, str), f"Failed to normalize private use char: {repr(char)}"
    ...
    def test_maximum_unicode_codepoints(self):
        ...
        for char in high_codepoints:
            try:
                normalized = self.normalizer.normalize(char)
                assert isinstance(normalized, str), f"Failed to normalize high codepoint: {repr(char)}"
                
                # Should detect script
                script = self.normalizer.detect_primary_script(char)
                assert script in ['Other', 'Unknown', 'Emoji'], f"Invalid script for high codepoint: {script}"
    ...
    def test_invalid_unicode_sequences(self):
        ...
        for seq in invalid_sequences:
            try:
                ...
                assert isinstance(normalized, str), f"Invalid sequence crashed normalization: {repr(seq)}"
