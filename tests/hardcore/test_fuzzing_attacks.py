...
from hypothesis.strategies import (binary, dictionaries, floats, integers,
                                   lists, text)
...
class TestInputFuzzing:
    ...
    def test_fuzz_unicode_normalization(self, malformed_name):
        ...
        try:
            ...
            assert any(word in str(e).lower() for word in ['invalid', 'malformed', 'unsupported']), \
                f"Unexpected exception type: {str(e)}"
    
    @given(malformed_entry=dictionaries(
    ...
    def test_fuzz_globalid_generation(self, malformed_entry):
        ...
        try:
            ...
            expected_errors = ['missing', 'invalid', 'malformed', 'unsupported', 'required', 'canonical', 'must have']
    ...
    def test_fuzz_yaml_parsing(self, malformed_yaml):
        ...
        try:
            ...
            assert any(word in str(e).lower() for word in yaml_errors), \
        ...
        for attack_bytes in binary_attacks:
            try:
                ...
                if normalized:
                    ...
                    assert '\ufffd' not in normalized or len(normalized) < len(attack_string), \
                ...
                if global_id:
                    assert validate_global_id(global_id), f"Invalid GlobalID from binary attack: {global_id}"
    ...
    def test_fuzz_zip_bomb_attacks(self):
        ...
        try:
            ...
            assert any(word in str(e).lower() for word in compression_errors), \
    ...
    def test_fuzz_regex_catastrophic_backtracking(self):
        ...
        for attack_pattern in backtracking_attacks:
            try:
                ...
                assert processing_time < 1.0, f"Possible regex backtracking attack: {processing_time}s"
    ...
    def test_fuzz_integer_overflow_attacks(self):
        ...
        for overflow_value in overflow_values:
            try:
                ...
                if global_id:
                    assert validate_global_id(global_id), f"Invalid GlobalID from overflow: {global_id}"
        ...
        for attack_string in normalization_attacks:
            try:
                ...
                if normalized:
                    ...
                    dangerous_chars = ['\u200B', '\u200C', '\u200D', '\u202E', '\u202D']
                    for char in dangerous_chars:
                        assert char not in normalized, f"Dangerous character not removed: {repr(char)}"
                ...
                if global_id:
                    assert validate_global_id(global_id), f"Invalid GlobalID from normalization attack"
                    
            except Exception as e:
                # Should handle gracefully
                assert "unicode" in str(e).lower() or "normalization" in str(e).lower(), \
            ...
            try:
                if isinstance(attack_data, str):
                    ...
                    if normalized:
                        ...
                        if global_id:
                            assert validate_global_id(global_id), "Invalid GlobalID from large string"
                ...
                assert any(word in str(e).lower() for word in memory_errors), \
    ...
    def test_fuzz_format_string_attacks(self):
        ...
        for attack_string in format_attacks:
            try:
                ...
                if normalized:
                    assert "{" not in normalized or "}" not in normalized or \
                           normalized == attack_string, "Format string interpreted"
        ...
        try:
            ...
            if normalized:
                ...
                for char in normalized:
                    ...
                    assert category not in ['Cc', 'Cf'], \
                        f"Dangerous control character preserved: {repr(char)}"
                        
        except Exception as e:
            # Should handle gracefully
            assert "control" in str(e).lower() or "invalid" in str(e).lower(), \
                f"Unexpected control character error: {str(e)}"
    
    @given(attack_dict=dictionaries(
