...
from src.core.errors import (AuthenticationError, CircuitBreaker,
                             CircuitBreakerOpenError, CircuitBreakerState,
                             ConfigurationError, DatabaseError, ErrorCollector,
                             ErrorContext, ErrorRecovery, ErrorSeverity,
                             GMNAPError, MaxRetriesExceededError, NetworkError,
                             RateLimitError, RecoveryStrategy,
                             ResourceExhaustedError, RetryableError,
                             SchemaError, SkippableError, UnicodeError,
                             ValidationError)
...
class TestCircuitBreakerResilience:
    ...
    def test_circuit_breaker_concurrent_access(self):
        ...
        assert protection_ratio > 0.1, f"Circuit breaker should protect more operations: {protection_ratio:.2%}"
    ...
    async def test_exponential_backoff_limits(self):
        ...
        with patch('asyncio.sleep', tracked_sleep):
            ...
            for retry_count in range(10):
                ...
                try:
                    await self.recovery._apply_strategy(RecoveryStrategy.RETRY_WITH_BACKOFF, 
                                                       NetworkError("Test error"), context)
...
class TestErrorContextLeaks:
    ...
    def test_error_context_memory_bounds(self):
        ...
        assert len(self.collector._errors) <= 1000, f"Too many errors stored: {len(self.collector._errors)}"
