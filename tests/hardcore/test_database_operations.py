...
class TestDatabaseIntegrity:
    ...
    def test_database_corruption_recovery(self):
        ...
        config = DatabaseConfig(db_path=str(self.db_path), use_duckdb=False)  # Use SQLite for this test
        ...
        try:
            ...
            assert "corrupt" in str(e).lower() or "malformed" in str(e).lower() or "database" in str(e).lower() or "disk" in str(e).lower()
    ...
    def test_concurrent_transaction_integrity(self):
        ...
        # Verify results
        assert total_successful == num_workers * iterations_per_worker, \
            f"Not all operations succeeded: {total_successful}/{num_workers * iterations_per_worker}"
        
        # Verify final count
        with DatabaseManager(config) as db:
            stats = db.get_statistics()
            expected_entries = 100 + (num_workers * iterations_per_worker)  # Initial + worker entries
    ...
    def test_large_dataset_performance(self):
        ...
        with DatabaseManager(config) as db:
            ...
            assert insert_time < 10.0, f"Insert too slow: {insert_time:.2f}s for {total_entries} entries"
            ...
            print(f"  Insert time: {insert_time:.2f}s ({total_entries/insert_time:.0f} entries/sec)")
...
class TestDatabaseMemoryManagement:
    ...
    def test_memory_leak_detection(self):
        ...
        config = DatabaseConfig(db_path=str(db_path), use_duckdb=False)  # Use SQLite for simpler testing
...
class TestDatabaseRecovery:
    ...
    def test_wal_mode_recovery(self):
        ...
        config = DatabaseConfig(
            db_path=str(db_path),
            use_duckdb=False,  # Use SQLite for WAL testing
            enable_wal=True
        )
        ...
        if wal_path.exists():
            ...
            crashed_config = DatabaseConfig(
                db_path=str(crashed_db),
                use_duckdb=False
            )
    ...
    def test_transaction_rollback(self):
        ...
        config = DatabaseConfig(
            db_path=str(db_path),
            use_duckdb=False  # Use SQLite for transaction testing
        )
