...
from src.authorities.base import (AuthorityData, AuthorityFetcher, FetchStatus,
                                  QuotaManager)
...
class TestAuthorityQuotaManagement:
    ...
    def test_concurrent_quota_tracking(self):
        ...
        async def run_test():
            ...
            remaining = self.quota_manager.get_remaining_quota(test_service)
            assert remaining == 864000 - 20, f"Should have consumed 20 requests, remaining: {remaining}"
    ...
    def test_concurrent_api_corruption(self):
        ...
        async def run_test():
            ...
            with patch('aiohttp.ClientSession.get', side_effect=mixed_response):
                ...
                successful = sum(1 for r in results if isinstance(r, type(results[0])) and r.status == FetchStatus.NOT_FOUND)
                failed = sum(1 for r in results if isinstance(r, type(results[0])) and r.status == FetchStatus.NETWORK_ERROR)
