...
class TestPipelineMemoryManagement:
    ...
    async def test_memory_usage_large_dataset(self):
        ...
        with patch.object(pipeline, "_load_authorities"), patch.object(
            pipeline, "_load_regions"
        ), patch.object(pipeline, "_verify_file_ownership"), patch.object(
            pipeline, "_check_licenses"
        ):
            ...
            assert (
                region_growth < 600
            ), f"Excessive memory usage after region detection: {region_growth}MB"
    ...
    async def test_memory_leaks_during_pipeline(self):
        ...
        if len(memory_measurements) > 1:
            ...
            assert (
                memory_trend < 50
            ), f"Memory leak detected: {memory_trend}MB growth across iterations"
    ...
    async def test_memory_cleanup_between_stages(self):
        ...
        with patch.object(pipeline, "_load_authorities"), patch.object(
            ...
            start_memory = self.process.memory_info().rss / 1024 / 1024  # MB
            await pipeline._stage_0_config()
            stage_memories.append(
                ("stage_0", self.process.memory_info().rss / 1024 / 1024 - start_memory)
            )
            ...
            await pipeline._stage_1_ingest(self.input_path)
            stage_memories.append(
                ("stage_1", self.process.memory_info().rss / 1024 / 1024 - stage1_start)
            )
            ...
            await pipeline._stage_2_detect_region()
            stage_memories.append(
                ("stage_2", self.process.memory_info().rss / 1024 / 1024 - stage2_start)
            )
            ...
            for stage_name, memory_usage in stage_memories:
                assert (
                    memory_usage < 200
                ), f"Excessive memory usage in {stage_name}: {memory_usage}MB"
        ...
        with patch.object(pipeline, "_load_authorities"), patch.object(
            ...
            try:
                ...
                if current_memory > 400:  # 400MB threshold
                    ...
                    ), "Should reduce chunk size under memory pressure"
    ...
    async def test_concurrent_pipeline_memory_safety(self):
        ...
        assert (
            memory_growth < 300
        ), f"Excessive memory usage from concurrent pipelines: {memory_growth}MB"
    ...
    async def test_memory_efficient_streaming(self):
        ...
        assert (
            len(pipeline._entries) == total_entries
        ), f"Not all entries processed: {len(pipeline._entries)}"
        ...
        def check_memory_limit():
            ...
            if current_memory > memory_limit:
                raise ResourceExhaustedError(
                    f"Memory limit exceeded: {current_memory}MB > {memory_limit}MB"
                )
    ...
    async def test_memory_optimization_strategies(self):
        ...
        for result in results:
            assert result["entries_processed"] == len(
                entries
            ), f"Not all entries processed: {result}"
            assert (
                result["memory_usage"] < 300
            ), f"Excessive memory usage: {result['memory_usage']}MB"
            assert (
                result["processing_time"] < 30
            ), f"Processing too slow: {result['processing_time']}s"
        ...
        assert (
            small_chunk_memory <= large_chunk_memory * 1.5
