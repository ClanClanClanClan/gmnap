    ...
    for class_name, instance in instances.items():
        ...
        for i, test_entry in enumerate(cases):
            ...
            try:
                ...
                results[class_name][f"case_{i+1}"] = {"success": False, "error": str(e)}
    ...
    for class_name, instance in instances.items():
        ...
        for i, test_entry in enumerate(edge_cases):
            ...
            try:
                ...
                print(
                    f"  ⚠️  Edge case {i+1} raised exception: {entry_desc} -> {type(e).__name__}: {e}"
                )
...
def test_performance(instances):
    ...
    # Generate test data
    test_entries = [{"CanonicalLatin": f"TestName{i}, Given{i}"} for i in range(100)]
