...
class TestCrossrefAdapter:
    ...
    def test_offline_returns_no_hit(self):
        result = asyncio.run(
            _make_adapter().enrich({"CanonicalLatin": "Euler, Leonhard"})
        )
    ...
    def test_online_with_mock(self, monkeypatch):
        ...
        mock_r.json.return_value = {
            "message": {
                ...
                "items": [
                    {
                        "DOI": "10.1234/test",
                        "author": [{"family": "Euler", "given": "L"}],
                        "subject": ["Mathematics"],
                        "container-title": ["J. Math"],
                    }
                ],
            }
        }
        ...
        adapter.ctx.cache = AsyncMock(
            get_json=AsyncMock(return_value=None), set_json=AsyncMock()
        )
