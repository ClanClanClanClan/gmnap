...
| A. Wiles | A1 (GB) | ✗ G1 (-es suffix) | ✓ A1 (ROR: Cambridge→GB) | ✓ A1 |
| T. Tao | E1 (AU) | ✓ E1 (shen surname) | ✗ A1 (ROR: Princeton→US) | ✓ A1* |
| J.-P. Serre | A2 (FR) | ✓ A2 (jean/pierre) | ✓ A2 (ROR: Sorbonne→FR) | ✓ A2 |
...
| A. Avila | G1 (BR) | ✗ C3 (no signal) | ✓ G1 (ROR: IMPA→BR) | ✓ G1 |
...
- **T. Tao** (born Australia, parents Chinese) works at UCLA. ROR gives US→A1. But his name is Chinese (E1). Name-only detection correctly gives E1 (via "shen" and "tao" E1 surname matches). The institution OVERRIDES the correct name-based detection.
...
"A. J. Wiles" gets detected as G1 (Latin America) because the surname "wiles" ends with "-es", which is a G1 (Hispanic) suffix pattern. The scorer gives G1 a surname_score of 2.50 (from the "-es" suffix), while A1 gets only 0.14 (from weak given-name fragments "andrew" and "john").
