...
def _require_extreme_and(env_key: str | None = None, expected: str | None = None):
    ...
    if env_key and expected is not None:
        if os.getenv(env_key) != expected:
...
class BaseExtreme:
    ...
    def _http(self, url: str, **params):
        if requests is None:
            raise RuntimeError("requests not available")
        r = requests.get(
            url, params=params, headers={"User-Agent": "gmnap-omega-bayes"}, timeout=30
        )
        r.raise_for_status()
...
class MathSciNet_HTML(BaseExtreme):
    name = "MathSciNet_HTML"  # Tier 2; subscription
    ...
    def query(self, q: str = "einstein"):
        ...
        # Endpoint requires subscription; we avoid hardcoding URL.
        return {"ok": True, "source": self.name, "data": "<html>MathSciNet</html>"}


class Scopus(BaseExtreme):
    name = "Scopus"  # Tier 2; Elsevier
...
class ProQuest_ETD(BaseExtreme):
    name = "ProQuest_ETD"  # Tier 3; commercial; optin
...
class Google_Scholar(BaseExtreme):
    name = "Google_Scholar"  # Tier 3; scraping; optin strict
    ...
    def query(self, accept_env="YES_I_ACCEPT_GS_TOS"):
        ...
        if OFFLINE:
            return self._offline()
        # Do not implement scraping here; contract test asserts gating & schema only.
