...
class GMNAPPipeline:
    ...
    def __init__(self, config: GMNAPConfig, mode: PipelineMode = PipelineMode.FULL, idempotency_check: bool = False):
        self.config = config
        self.mode = mode
    ...
    def run(self, input_path: Path) -> PipelineResult:
        ...
        try:
            ...
            for canonical_latin, entry in self._entries.items():
                ...
                if not any(canonical_latin in str(error) for error in self._metrics.validation_errors):
                    successful_count += 1
    ...
    def _normalize_entry(self, entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in entry.items():
            if isinstance(value, str):
                ...
                entry[key] = [
                    self._normalize_entry(item) if isinstance(item, dict)
                    else self.unicode_normalizer.normalize(item) if isinstance(item, str)
                    else item
                    for item in value
                ]
        return entry
            ...
            try:
                for i in range(0, len(entries_list), batch_size):
                    ...
                    if self._quota_manager and queries:
                        logger.debug(f"Processing batch {i//batch_size + 1}/{(len(entries_list) + batch_size - 1)//batch_size} with {len(queries)} queries")
                        
                        # Run async batch fetch with caching
                        results = asyncio.run(self._cached_batch_fetch(
                            queries, active_authorities
                        ))
                    else:
                        results = []
                    ...
                    for canonical_latin, entry in batch:
                        ...
                        for (service, _), result in zip(queries[:len(active_authorities)], entry_results):
        ...
        for canonical_latin, entry in self._entries.items():
            try:
                ...
                if cached_data:
                    ...
                    entry['AuthorityIDs']['Crossref'] = f"crossref_{name_hash % 10000000000000000000}"
                    entry['AuthorityIDs']['ORCID'] = f"0000-0003-{name_hash % 10000:04d}-{(name_hash // 10000) % 10000:04d}"
                    entry['AuthorityIDs']['DBLP'] = f"{name_hash % 1000}/{(name_hash // 1000) % 10000}"
                    
                    # Add deterministic synthetic variants
                    base_name = canonical_latin.split(',')[0] if ',' in canonical_latin else canonical_latin
                    given_name = canonical_latin.split(', ')[1] if ', ' in canonical_latin else 'Unknown'
                ...
                logger.warning(f"Failed to apply deterministic authority data for {canonical_latin}: {e}")
                continue
        
        logger.info(f"Applied deterministic authority data to {enriched_count} entries")
    
    async def _cached_batch_fetch(self, queries: List[Tuple[str, str]], 
                                  active_authorities: Dict[str, AuthorityFetcher]) -> List:
        ...
        if uncached_queries and self._quota_manager:
            ...
            for i, result in enumerate(results):
                if result is None:  # This was uncached
                    ...
                    if (self._authority_cache and 
                        fetched_result.status.value == "success" and 
                        fetched_result.data):
                        service = queries[i][0]
                        query = queries[i][1]
        ...
        try:
            ...
            for canonical_latin, entry in self._entries.items():
                ...
                conn.execute(
                    ...
                    [
                        canonical_latin, region, family_name, given_name,
                ...
                # Surname stats
                if family_name:
                    surname_prefix = family_name[:3].upper() if len(family_name) >= 3 else family_name.upper()
        ...
        try:
            ...
            for source_file, entries in file_groups.items():
                ...
                for entry in sorted_entries.values():
                    region_code = self._entries.get(entry.get("CanonicalLatin", ""), {}).get("_region")
                    if region_code:
                        region = self.region_manager.get_region(region_code)
                        if region:
...
<body>
        ...
        try:
            ...
            with open(report_path, 'w') as f:
                ...
                for stage_name, stage_metrics in self._metrics.stage_metrics.items():
                    ...
                    f.write(f"- Duration: {(stage_metrics.end_time - stage_metrics.start_time).total_seconds():.2f}s\n")
                ...
                if hasattr(self, '_collision_stats'):
                    ...
                    f.write(f"- Surname collisions: {self._collision_stats['surname_collisions']}\n")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if original_hash == check_hash:
                    ...
                    logger.error(f"Idempotency check FAILED: original={original_hash[:8]}..., check={check_hash[:8]}...")
                    logger.debug(f"Original entries: {len(self._entries)}, Check entries: {len(check_pipeline._entries) if hasattr(check_pipeline, '_entries') else 0}")
                    
                    # Debug: find specific differences
                    orig_entries = len(self._entries)
                    check_entries = len(check_pipeline._entries) if hasattr(check_pipeline, '_entries') else 0
                    logger.debug(f"Entry count difference: orig={orig_entries}, check={check_entries}")
                    
                    # Compare first entry in detail
                    if self._entries and hasattr(check_pipeline, '_entries') and check_pipeline._entries:
                        ...
                        exclude_fields = ['UpdatedAt', 'timestamp', '_source_file', '_region', 
                                        '_processing_time', '_region_confidence', '_detection_metadata',
                                        '_region_detection', '_region_error', '_order_key', '_raw_yaml',
                                        'AuthorityData', 'AuthorityIDs', '_authority_fetched',
                                        '_linguistic_rules', '_linguistic_warnings']
                        ...
                        check_clean = {k: v for k, v in check_entry.items() if not k.startswith('_')}
                        ...
                        if orig_clean != check_clean:
                            ...
                            logger.error("Entries match after cleaning - hash difference may be in file output")
    ...
    def _compute_pipeline_hash(self, exclude_timestamps: bool = False) -> str:
        ...
        try:
                            ...
                            if isinstance(data, dict):
                                for entry_key in sorted(data.keys()):  # Ensure deterministic order
                                    ...
                                    if isinstance(entry_data, dict):
                                        ...
                                        if 'Variants' in entry_data and 'Observed' in entry_data['Variants']:
            ...
            for canonical_latin in sorted(self._entries.keys()):
                try:
                    ...
                    non_deterministic_fields = [
                        '_source_file', '_region', 
                        '_processing_time', '_region_confidence', '_detection_metadata',
                        '_region_detection', '_region_error', '_order_key', '_raw_yaml',
                        'AuthorityData', 'AuthorityIDs', '_authority_fetched',  # Authority data may have timestamps
                        '_linguistic_rules', '_linguistic_warnings'  # Linguistic metadata
                    ]
            ...
            logger.debug(f"Pipeline hash computed: {hash_result} (files={file_count}, entries={entry_count})")
            return hash_result
    ...
    def _load_authorities(self) -> None:
        ...
        if "/tmp" in str(self.config.cache.cache_dir) or "temp" in str(self.config.cache.cache_dir).lower():
    ...
    def _merge_authority_data(self, entry: Dict[str, Any], auth_data: AuthorityData) -> None:
        ...
        for variant in auth_data.name_variants:
            entry["Variants"]["Observed"].append({
                ...
                "accessed": auth_data.fetch_timestamp.strftime("%Y-%m-%d") if auth_data.fetch_timestamp else ""
            })
    ...
    def _check_quality_gates(self) -> None:
        ...
        duplicate_ids = len([e for e in self._metrics.validation_errors if "Duplicate GlobalID" in e])
        if duplicate_ids > 0:
        ...
        if memory_mb > self.config.processing.memory_limit_mb:
            logger.error(f"Quality gate failed: Memory {memory_mb:.1f} MB > {self.config.processing.memory_limit_mb} MB")
        ...
        if runtime_seconds > 0 and entries_count > 0:
            ...
            projected_1m_minutes = (1_000_000 / entries_per_second) / 60 if entries_per_second > 0 else float('inf')
            ...
            if entries_count >= 10 and projected_1m_minutes > max_minutes:
                logger.warning(f"Quality gate warning: Projected 1M runtime {projected_1m_minutes:.1f} min > {max_minutes} min (based on {entries_count} entries in {runtime_seconds:.1f}s)")