...
class MemgraphClient:
    """Minimal Memgraph wiring with MAGE/Neo4j-style betweenness fallbacks.
    If Bolt is unavailable, methods return offline-safe defaults.
    """
        ...
        try:
            ...
            for e in entries:
                ...
                for adv in e.get("Advisors", []) or []:
                    tx.run(
                        """                        MATCH (a:Mathematician {gid:$a}), (s:Mathematician {gid:$s})
                        MERGE (a)-[:ADVISED]->(s)
                    """,
                        a=adv,
                        s=sid,
