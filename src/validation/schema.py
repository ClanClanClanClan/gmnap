...
class SchemaValidator:
    ...
    def _custom_validations(self, entry_data: Dict[str, Any]) -> List[str]:
        ...
        for canonical_latin, entry in entry_data.items():
            ...
            if entry.get("CanonicalLatin") != canonical_latin:
                errors.append(f"CanonicalLatin mismatch: key='{canonical_latin}', value='{entry.get('CanonicalLatin')}'")
            ...
            valid_types = {
                "ascii-lossy", "tone-number", "diac-drop", 
                "order-swap", "romanisation-alt", "particle-drop", 
                "initial-collapse"
            }
            ...
            for i, affiliation in enumerate(timeline):
                if "country" not in affiliation:
                    ...
                    errors.append(f"Invalid country in AffiliationTimeline[{i}]: {affiliation['country']}")
                ...
                if from_year is not None and to_year is not None:
                    if to_year < from_year:
                        errors.append(f"AffiliationTimeline[{i}]: to ({to_year}) before from ({from_year})")
    ...
    def validate_multiple_entries(self, entries: List[Dict[str, Any]]) -> Dict[str, Tuple[bool, List[str]]]:
        """
...
def validate_entry(entry_data: Dict[str, Any], schema_path: Optional[str] = None) -> Tuple[bool, List[str]]:
    """
