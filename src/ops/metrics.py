...
def Gauge(name: str, desc: str = "", labelnames: list = None):
    if _PGauge is None: return _NoOp()
    if labelnames:
        return _PGauge(name, desc, labelnames=labelnames)
    return _PGauge(name, desc)

# Common metrics used across stages
WRITE_DIFF_CHANGED_ENTRIES = Gauge("gmnap_write_diff_changed_entries", "Changed entries reported by Stage 9")
...
# Stage 8 metrics
SCHEMA_VALIDATION_ERRORS = Counter("gmnap_schema_validation_errors_total", "Schema validation errors", labelnames=["field"])
ROUNDTRIP_FAILURES = Counter("gmnap_roundtrip_failures_total", "Roundtrip validation failures", labelnames=["region"])
...
# Archive metrics
ARCHIVE_UPLOADS_SUCCEEDED = Counter("gmnap_archive_uploads_succeeded_total", "Archive uploads succeeded")
