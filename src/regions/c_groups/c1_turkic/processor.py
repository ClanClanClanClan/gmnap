...
class C1TurkicProcessor(RegionSpec):
    ...
    def __init__(self):
        ...
        self.script_reforms = {
            ...
            "KZ": {"start": 2023, "end": 2031, "status": "transitional", "from": "Cyrillic", "to": "Latin"},
        ...
        self.turkic_latin_chars = {
            ...
            'ə', 'Ə',
            # Uzbek
            'oʻ', 'gʻ', 'Oʻ', 'Gʻ', ''', ''',  # Apostrophe variants
            # Turkmen
            'ä', 'Ä', 'ň', 'Ň', 'ý', 'Ý', 'ž', 'Ž',
        ...
        self.turkic_cyrillic_chars = {
            ...
            'ә', 'Ә', 'ғ', 'Ғ', 'қ', 'Қ', 'ң', 'Ң', 'ө', 'Ө', 'ұ', 'Ұ', 'ү', 'Ү', 'һ', 'Һ',  # Kazakh
            'ө', 'Ө', 'ү', 'Ү', 'ң', 'Ң',  # Kyrgyz
            'ҳ', 'Ҳ', 'ў', 'Ў', 'қ', 'Қ', 'ғ', 'Ғ', 'ҳ', 'Ҳ',  # Uzbek
            'ә', 'Ә', 'җ', 'Җ', 'ң', 'Ң', 'ө', 'Ө', 'ү', 'Ү', 'һ', 'Һ', 'ы', 'Ы'  # Turkmen
        }
        ...
        self.common_turkic_names = {
            # Turkish
            "Mehmet", "Ahmet", "Mustafa", "Ali", "Hasan", "Hüseyin", "İbrahim", "Osman",
            "Ayşe", "Fatma", "Emine", "Hatice", "Zeynep", "Şerife", "Meryem",
            # Azerbaijani
            "Əli", "Həsən", "Hüseyn", "Məhəmməd", "Rəşid", "Elçin",
            "Günel", "Leyla", "Sevil", "Aynur", "Nigar",
            # Central Asian
            "Bakyt", "Bolot", "Ermek", "Nursultan", "Aida", "Ainur", "Gulnara",
        }
        ...
        self.titles = {
            # Turkish
            "Dr", "Dr.", "Prof", "Prof.", "Doç", "Doç.", "Bay", "Bayan", "Sayın",
            # Azerbaijani
            "Cənab", "Xanım",
            # Central Asian
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        if components.get("likely_country") == "KZ" or self._is_kazakh_name(canonical):
            kazakh_script_variants = self._generate_kazakh_script_transition_variants(canonical, components)
    ...
    def _extract_components(self, name: str) -> Dict[str, Any]:
        ...
        if country:
            ...
            components["script_reform_status"] = self.script_reforms.get(country, {}).get("status", "stable")
        
        return components
    
    def _detect_script(self, name: str) -> str:
        """Detect the primary script used in the name."""
        latin_count = sum(1 for c in name if c in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" or c in self.turkic_latin_chars)
    ...
    def _generate_script_variants(self, name: str, components: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if current_script in ["Cyrillic", "Arabic"]:
            ...
            if ascii_name != name:
                variants.append({
                    "str": ascii_name,
                    "type": f"{current_script.lower()}-to-latin"
                })
    ...
    def _generate_turkish_case_variants(self, name: str) -> List[Dict[str, str]]:
        ...
        if international_variant != name:
            variants.append({
                "str": international_variant,
                "type": "international-compatible"
            })
    ...
    def _is_kazakh_name(self, name: str) -> bool:
        ...
        kazakh_cyrillic_chars = {'ә', 'ғ', 'қ', 'ң', 'ө', 'ұ', 'ү', 'һ', 'Ә', 'Ғ', 'Қ', 'Ң', 'Ө', 'Ұ', 'Ү', 'Һ'}
    ...
    def _generate_kazakh_script_transition_variants(self, name: str, components: Dict[str, Any]) -> List[Dict[str, str]]:
        """
        ...
        kazakh_cyrillic_to_latin = {
            ...
            'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'с': 's', 'т': 't', 'у': 'u',
            'ф': 'f', 'х': 'h', 'ц': 'ts', 'ч': 'ş', 'ш': 'ş', 'щ': 'şş',
            'ъ': '', 'ы': 'ı', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya',
            
            # Kazakh-specific Cyrillic → Latin
            'ә': 'ä', 'ғ': 'ğ', 'қ': 'q', 'ң': 'ñ', 'ө': 'ö', 'ұ': 'u', 'ү': 'ü', 'һ': 'h',
        ...
        kazakh_latin_to_cyrillic.update({
            'ä': 'ә', 'ğ': 'ғ', 'q': 'қ', 'ñ': 'ң', 'ö': 'ө', 'ü': 'ү',
            'Ä': 'Ә', 'Ğ': 'Ғ', 'Q': 'Қ', 'Ñ': 'Ң', 'Ö': 'Ө', 'Ü': 'Ү',
            'ı': 'ы', 'I': 'Ы'  # Dotless i maps to Kazakh ы
        })
        
        if current_script == "Cyrillic":
            # Generate Latin variant
            latin_variant = self._transliterate_kazakh_cyrillic_to_latin(name, kazakh_cyrillic_to_latin)
            ...
            cyrillic_variant = self._transliterate_kazakh_latin_to_cyrillic(name, kazakh_latin_to_cyrillic)
    ...
    def _transliterate_kazakh_cyrillic_to_latin(self, name: str, mapping: Dict[str, str]) -> str:
        ...
        transliterated = re.sub(r'([A-ZÄÖÜĞÑŞİI])([a-zäöüğñşıi])', r'\1\2', transliterated)  # Fix case
    ...
    def _extract_turkic_patronymic_details(self, name: str, components: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        turkic_patronymic_map = {
            # Turkish
            "oğlu": {"meaning": "son of", "gender": "masculine", "language": "Turkish", "country": "TR"},
            "kızı": {"meaning": "daughter of", "gender": "feminine", "language": "Turkish", "country": "TR"},
            
            # Azerbaijani (similar to Turkish but distinct)
            "qızı": {"meaning": "daughter of", "gender": "feminine", "language": "Azerbaijani", "country": "AZ"},
            
            # Uzbek (with apostrophes)
            "o'g'li": {"meaning": "son of", "gender": "masculine", "language": "Uzbek", "country": "UZ"},
            "qizi": {"meaning": "daughter of", "gender": "feminine", "language": "Uzbek", "country": "UZ"},
            
            # Turkmen
            "ogly": {"meaning": "son of", "gender": "masculine", "language": "Turkmen", "country": "TM"},
            "gyzy": {"meaning": "daughter of", "gender": "feminine", "language": "Turkmen", "country": "TM"},
            
            # Kyrgyz
            "uulu": {"meaning": "son of", "gender": "masculine", "language": "Kyrgyz", "country": "KG"},
            "kyzy": {"meaning": "daughter of", "gender": "feminine", "language": "Kyrgyz", "country": "KG"},
            
            # Kazakh (Cyrillic)
            "ұлы": {"meaning": "son of", "gender": "masculine", "language": "Kazakh", "country": "KZ"},
            "қызы": {"meaning": "daughter of", "gender": "feminine", "language": "Kazakh", "country": "KZ"},
        }
    ...
    def _generate_patronymic_variants(self, name: str, components: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        if not self._is_valid_turkic_name(canonical):
            invalid_chars = set(canonical) - set("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ,.-'") - self.allowed_chars
            if invalid_chars:
    ...
    def _normalize_for_sorting(self, text: str) -> str:
        ...
        replacements = {
            'ç': 'czz', 'Ç': 'CZZ',
            'ğ': 'gzz', 'Ğ': 'GZZ',
            'ı': 'izz', 'I': 'Izz',  # Turkish dotless i
            'İ': 'I', 'i': 'i',     # Turkish dotted i
            'ö': 'ozz', 'Ö': 'OZZ',
            'ş': 'szz', 'Ş': 'SZZ',
            'ü': 'uzz', 'Ü': 'UZZ',
            'ə': 'azz', 'Ə': 'AZZ',  # Azerbaijani
        }
