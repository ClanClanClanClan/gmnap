...
class C2_PersianTajik(RegionSpec):
    ...
    def _analyze_persian_ezafe(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        if len(words) == 2:
            first, second = words
            first = str(first)  # Ensure string type
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        # V7 SECURITY: Check for dangerous characters and DoS protection FIRST
        ...
        # Check for invalid characters (more permissive)
        ...
        if canonical_native and len(canonical_native.strip()) > 0:
            if not self._is_persian_or_tajik(canonical_native):
                # Only warn for non-Persian/Cyrillic native names in C2 region
                self.logger.warning(
                    f"CanonicalNative in C2 region is not Persian/Cyrillic: {canonical_native}"
                )
