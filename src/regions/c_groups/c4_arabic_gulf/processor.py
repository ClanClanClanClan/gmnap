...
class C4_ArabicGulf(RegionSpec):
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        for variant in al_variants:
            if variant != canonical:
                self.add_variant(entry, {"str": variant, "type": "al-variant"})
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        # If CanonicalNative exists, it should ideally be Arabic (but be lenient)
        if canonical_native:
            if not self._is_arabic(canonical_native) and not self._is_mixed_script(
                ...
                self.logger.warning(
                    f"CanonicalNative in C4 region is not Arabic: {canonical_native}"
                )
    ...
    def _has_valid_characters(self, name: str) -> bool:
        ...
        for char in name:
            # Allow Latin, Arabic, spaces, hyphens, apostrophes, periods, commas
