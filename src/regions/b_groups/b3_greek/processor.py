...
class B3GreekProcessor(RegionSpec):
    ...
    def __init__(self):
        ...
        # Greek diacritics
        self.greek_diacritics = set(
            "άέήίόύώΆΈΉΊΌΎΏΐΰϊϋΐΰ"
        )
        ...
        self.romanised_prefixes = {
            ...
            "Chatzi", "Hatzi", "Chatzi-", "Hatzi-", "Hadji", "Haji",
            # Papa- variants
            "Papa", "Papas", "Papa-",
            # Others
            "Kara", "Kara-", "Mastro", "Mastro-",
            "Kyria", "Kyr", "Gero", "Makro", "Mikro"
        }
        
        # Common Greek surnames (for pattern detection)
        self.common_greek_surnames = {
            # Most common
            "Παπαδόπουλος", "Papadopoulos", "Παπαδοπούλου",
            "Παναγιωτόπουλος", "Panagiotopoulos",
            "Γιαννόπουλος", "Giannopoulos", "Yannopoulos",
            "Νικολάου", "Nicolaou", "Nikolaou",
            "Κωνσταντίνου", "Constantinou", "Konstantinou",
            # Cypriot common
            "Χριστοδούλου", "Christodoulou",
            "Γεωργίου", "Georgiou",
            "Μιχαήλ", "Michael", "Michail",
            # With prefixes
            "Χατζηδάκης", "Hadjidakis", "Chatzidakis",
            "Παπαγεωργίου", "Papageorgiou"
        }
        
        # Genitive endings (common in surnames)
        self.genitive_endings = {
            # Male genitive
            "-ου", "-όπουλος", "-άκης", "-ίδης", "-άδης",
            # Female forms
            "-ού", "-οπούλου", "-άκη", "-ίδου", "-άδου"
        }
        
        # Ancient Greek indicators (for rule #25)
        self.ancient_indicators = {
            # Names
            "Αριστοτέλης", "Aristotle", "Πλάτων", "Plato",
            "Σωκράτης", "Socrates", "Ευκλείδης", "Euclid",
            "Αρχιμήδης", "Archimedes", "Πυθαγόρας", "Pythagoras",
            # Patterns
            "-κλής", "-kles", "-μένης", "-menes",
            "-φάνης", "-phanes", "-γένης", "-genes"
        }
        
        # Titles to remove
        self.titles = {
            # Greek
            "Κύριος", "Κυρία", "Δρ", "Δρ.", "Καθ", "Καθ.",
            # English (common in Cyprus)
            "Mr", "Mr.", "Mrs", "Mrs.", "Ms", "Ms.", "Dr", "Dr.",
            "Prof", "Prof.", "Professor",
            # Academic
            "Καθηγητής", "Καθηγήτρια", "Δόκτωρ"
        }
    ...
    def _is_ancient_greek(self, latin: str, greek: str) -> bool:
        ...
        ancient_patterns = ["-kles", "-menes", "-phanes", "-genes", 
                           "-κλής", "-μένης", "-φάνης", "-γένης"]
