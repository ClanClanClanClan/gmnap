...
class B2_SouthSlavicCentral(RegionSpec):
    ...
    def __init__(self):
        ...
        self.cyrillic_chars = set('абвгдежзијклмнопрстуфхцчшщъыьэюяАБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ')
        
        # Combined Latin character set for the region
        self.latin_extended = (self.polish_chars | self.czech_slovak_chars | 
                              self.south_slavic_latin | self.romanian_chars)
        
        # Country mapping with specific features
        self.country_features = {
            'PL': {'chars': self.polish_chars, 'suffixes': ['ski', 'ska', 'cki', 'cka', 'icz', 'owicz', 'ewicz']},
            'CZ': {'chars': self.czech_slovak_chars, 'suffixes': ['ová', 'ý', 'á', 'ek', 'ík']},
            'SK': {'chars': self.czech_slovak_chars, 'suffixes': ['ová', 'ý', 'á', 'ek', 'ík', 'iak']},
            ...
            'RS': {'scripts': ['Cyrillic', 'Latin'], 'suffixes': ['ић', 'овић', 'евић', 'ić', 'ović', 'ević']},
            ...
            'XK': {'scripts': ['Latin'], 'suffixes': ['i', 'u', 'aj', 'ić']}  # Albanian/Serbian mix
        }
        ...
        self.hungarian_patterns = {
            ...
            'common_families': ['Nagy', 'Kovács', 'Tóth', 'Szabó', 'Horváth', 'Varga', 'Kiss', 'Molnár', 'Németh', 'Farkas']
        }
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        if invalid_chars:
            raise RegionRuleError(f"Invalid characters for {script} script: {', '.join(invalid_chars)}")
    ...
    def _is_hungarian_name(self, name: str) -> bool:
        ...
        if any(first_word.endswith(suffix) for suffix in self.hungarian_patterns['family_suffixes']):
            return True
    ...
    def _estimate_country(self, name: str, script: str) -> Optional[str]:
        ...
        # Romanian
        if any(c in name for c in 'ăîșțĂÎȘȚ') or any(name.endswith(suffix) for suffix in ['escu', 'eanu']):
            return 'RO'
