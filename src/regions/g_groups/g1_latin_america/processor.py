...
class G1_LatinAmerica(RegionSpec):
    ...
    def clean(self, entry: Dict[str, Any]) -> None:
        ...
        if not canonical:
            # More graceful handling - check for CanonicalNative
            canonical = entry.get("CanonicalNative", "")
            if not canonical:
                # Both empty - skip cleaning but don't fail
                return
        ...
        for pattern_name, pattern in self.compound_patterns.items():
            if pattern.search(cleaned):
                # Ensure compound names are properly spaced using base class method
                cleaned = self.normalize_whitespace_characters(cleaned)
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        if not canonical:
            # No processable name - skip augmentation
            return
        ...
        # Initialize variants (use standard structure)
        ...
        entry["VariantsSynthesised"] = entry["Variants"]["Synthesised"]
        ...
        if surname_components:
            ...
            if surname_components.get("secondary"):
                ...
                if hasattr(self, "add_variant"):
                    ...
                    self.add_variant(
                        entry, {"str": secondary_only, "type": "dual-surname-secondary"}
                    )
        ...
        if ", " in canonical:
            ...
            if hasattr(self, "add_variant"):
                ...
                self.add_variant(entry, {"str": swapped, "type": "order-swap"})
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        """
        Validate Latin American names using standardized validation.
        """
        # Apply comprehensive security and validation checks from base class
        self.apply_security_and_validation_checks(entry)

        # G1-specific validation: Dual surname and compound name structure
        canonical = self.get_canonical_name(entry)
        if not canonical:
            return

        # Optional validation for dual surname structure (warning only, not error)
        ...
        for compound in compound_names:
            if "  " in compound:  # Double spaces
                self.logger.warning(f"Malformed compound name spacing in: {compound}")
