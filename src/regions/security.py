...
class SecurityFilter:
    ...
    def __init__(self):
        ...
        self.attack_strings = {
            # XSS vectors
            'alert(', 'prompt(', 'confirm(', 'document.', 'window.', 'eval(',
            'function(', 'settimeout', 'setinterval',
            
            # SQL injection
            '\' or \'', '" or "', '\' union ', '" union ', 'drop table',
            'select * from', 'insert into', 'update set', 'delete from',
            
            # Command injection
            '/bin/', '/usr/bin/', 'cmd.exe', 'powershell', 'bash',
            ...
            # Common payloads
            '<svg/onload=', '<img/src=x/onerror=', '<?php', '<%=', '<%= %>',
    ...
    def secure_clean_name(self, name: str, field_name: str = "name") -> str:
        ...
        if not isinstance(name, str):
            raise SecurityError(f"Name field {field_name} must be string, got {type(name).__name__}")
