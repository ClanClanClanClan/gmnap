...
class A4OceaniaProcessor(RegionSpec):
    ...
    def __init__(self):
        ...
        self.macron_chars = {
            'ā', 'Ā', 'ē', 'Ē', 'ī', 'Ī', 'ō', 'Ō', 'ū', 'Ū',
            'ă', 'Ă', 'ĕ', 'Ĕ', 'ŏ', 'Ŏ'  # Some regions use breve
        }
        
        # Other special characters used in the region
        self.special_chars = {
            "'", "'", "ʻ",  # ʻokina (glottal stop)
            "ç", "Ç",       # French influence (New Caledonia)
            "ñ", "Ñ"        # Some Spanish influence
        }
        ...
        self.polynesian_particles = {
            # Māori
            "Te", "Ngā", "Ngāi", "Ngāti", "Ka", "Tū",
            # Samoan  
            "O", "Le", "Sa", "Fa'a", "Faa",
            # Tongan
            "Tu'i", "Tui", "'Aho", "Aho", "Faka",
            # Fijian
            "Ro", "Adi", "Ratu", "Tui",
        ...
        self.common_elements = {
            ...
            "moana", "vai", "mata", "rangi", "lani", "marama",
            "tane", "wahine", "keiki", "tamariki",
            # Directional/locational
            "hema", "tonga", "tokelau", "hiva",
            # Status/nobility markers
            "ariki", "ali'i", "alii", "matai", "tulafale"
        }
        
        # Colonial-origin surnames (English/French)
        self.colonial_surnames = {
            # English
            "Smith", "Williams", "Brown", "Wilson", "Taylor",
            "Johnson", "White", "Martin", "Thompson", "Harris",
            # French
        ...
        self.polynesian_first_names = {
            # Māori
            "Aroha", "Hine", "Mere", "Pania", "Tane", "Wiremu",
            # Samoan
            "Tala", "Sina", "Malia", "Pua", "Tui", "Sione",
            # Tongan
            "Salote", "Finau", "Viliami", "Siaosi", "Mele",
            # Fijian
            "Mere", "Salote", "Litia", "Vasiti", "Jone", "Peni"
        }
        ...
        self.titles = {
            ...
            "Tui", "Tu'i", "Ratu", "Adi", "Ro"  # Often integral to names
        }
    ...
    def _remove_titles(self, text: str) -> str:
        ...
        while words and words[0] in self.titles:
            ...
            if words[0] in ["Tui", "Tu'i", "Ratu", "Adi", "Ro"] and len(words) > 1:
                ...
                if (len(words) > 1 and 
                    (words[1] in self.polynesian_first_names or 
                     words[1] in self.colonial_surnames)):
                    words.pop(0)
                else:
    ...
    def _detect_colonial_pattern(self, name: str) -> bool:
        ...
        for word in words:
            if (any(char in word for char in self.macron_chars) or
                'ʻ' in word or
                word in self.polynesian_particles or
                word.lower() in self.common_elements):
                has_polynesian = True
                break
