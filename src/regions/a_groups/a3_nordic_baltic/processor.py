...
class A3NordicBalticProcessor(RegionSpec):
    ...
    def __init__(self):
        ...
        self.scandinavian_particles = {
            ...
            "von",
            "von der",  # German influence
            "zu",  # German "to/at"
            "de",
            "de la",  # French influence
        ...
        if components.get("particle"):
            ...
            if particle_drop and particle_drop != canonical:
                entry["Variants"]["Synthesised"].append(
                    {"str": particle_drop, "type": "particle-drop"}
                )
    ...
    def _has_baltic_ending(self, name: str) -> bool:
        ...
        return any(name.endswith(ending) for ending in self.baltic_male_endings) or any(
            name.endswith(ending) for ending in self.baltic_female_endings
        )
    ...
    def _generate_particle_drop_variant(
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        if components.get("gender_inflected"):
            ...
            if gender == "female" and not any(
                family.endswith(e) for e in self.baltic_female_endings
            ):
                raise RegionRuleError(
                    f"Gender mismatch: marked female but no female ending: {family}"
                )
