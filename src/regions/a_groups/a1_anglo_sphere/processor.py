...
class A1_AngloSphere(RegionSpec):
    ...
    def is_applicable(self, entry: Dict[str, Any]) -> bool:
        ...
        if detected_region == "A1":
            return True

        # Check canonical forms for Latin/ASCII text
        canonical_latin = entry.get("CanonicalLatin", "")
        canonical_native = entry.get("CanonicalNative", "")

        # If we have Latin text, check if it's ASCII/extended Latin
        for canonical in [canonical_latin, canonical_native]:
            if canonical and self._is_likely_anglo_name(canonical):
                return True

        return False

    def _is_likely_anglo_name(self, name: str) -> bool:
        """Check if name is likely from Anglo-Sphere region."""
        if not name:
            return False

        # Must be primarily Latin script
        if not self._is_valid_extended_latin(name):
            return False

        # Common Anglo patterns
        # 1. Contains typical Anglo particles
        words = name.lower().split()
        anglo_particles = {"mc", "mac", "o'", "de", "van", "von"}
        if any(word in anglo_particles for word in words):
            return True

        # 2. Contains typical Anglo suffixes
        if any(suffix in name for suffix in ["Jr.", "Sr.", "III", "II"]):
            return True

        # 3. Looks like "Family, Given" format (Anglo convention)
        if "," in name:
            parts = name.split(",")
            if len(parts) == 2 and parts[0].strip() and parts[1].strip():
                return True

        # 4. Contains middle initials (Anglo pattern)
        words = name.split()
        if len(words) >= 3:
            # Check for middle initials like "John A. Smith"
            for word in words[1:-1]:  # Middle words
                if len(word) <= 2 and word.endswith("."):
                    return True

        # 5. Default to True for basic Latin names (A1 is fallback for Latin)
        return True
    ...
    def clean(self, entry: Dict[str, Any]) -> None:
        ...
        for field in ["CanonicalLatin", "CanonicalNative"]:
            if field in entry and entry[field]:
                ...
                # Normalize tabs, newlines, and carriage returns (V7 edge case) BEFORE security check
                if "\t" in raw_input:
                ...
                # Check remaining dangerous chars using lenient method
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        # Ensure idempotency by calling parent
        super().augment(entry)

        # Check if we should continue (parent may have returned early)
        ...
        if ascii_variant != canonical:
            # Use add_variant if available, otherwise direct append
            if hasattr(self, "add_variant"):
                self.add_variant(entry, {"str": ascii_variant, "type": "ascii-lossy"})
            else:
                # Avoid duplicates
                existing = [v.get("str") for v in entry["Variants"]["Synthesised"]]
                if ascii_variant not in existing:
                    entry["Variants"]["Synthesised"].append(
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        # SECURITY: Apply base security validation first (includes path traversal protection)
        self.apply_security_and_validation_checks(entry)

        # Check for valid characters (Latin ASCII expected, but allow extended Latin)
        if not self._is_valid_ascii_name(canonical):
            # Check if it's extended Latin that's acceptable (more permissive for international names)
            if not self._is_valid_extended_latin(canonical):
                # Final check: allow common punctuation and international characters
                if not self._is_valid_international_name(canonical):
        ...
        if family and len(family) < 2:
            raise RegionRuleError("Family name too short")
    ...
    def _is_valid_international_name(self, name: str) -> bool:
        """Check if name contains valid international characters for Anglo-Sphere region."""
        # For A1 (Anglo-Sphere), be more restrictive than general international
        import unicodedata

        for char in name:
            category = unicodedata.category(char)
            # Allow only Latin letters (Ll, Lu, Lt), not all letters
            if category in ("Ll", "Lu", "Lt"):  # Latin lowercase, uppercase, titlecase
                continue
            # Allow marks/diacritics for names like José, François
            if category.startswith("M"):
                continue
            # Allow spaces
            if category == "Zs":
                continue
            # Allow common name punctuation: apostrophes, hyphens, periods, commas
            if char in "'-., ":
                continue
            # Allow numbers in some contexts (like "John III", "2nd Street")
            if category == "Nd":
                continue
            # Reject everything else including CJK, Arabic, etc.
            return False
