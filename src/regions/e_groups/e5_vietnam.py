...
class E5Vietnam(RegionSpec):
    ...
    def __init__(self):
        ...
        self.tone_mappings = {
            # Vowel 'a'
            'a': ('a', 1), 'á': ('a', 2), 'à': ('a', 3), 'ả': ('a', 4), 'ã': ('a', 5), 'ạ': ('a', 6),
            'A': ('A', 1), 'Á': ('A', 2), 'À': ('A', 3), 'Ả': ('A', 4), 'Ã': ('A', 5), 'Ạ': ('A', 6),
            
            # Vowel 'e'
            'e': ('e', 1), 'é': ('e', 2), 'è': ('e', 3), 'ẻ': ('e', 4), 'ẽ': ('e', 5), 'ẹ': ('e', 6),
            'E': ('E', 1), 'É': ('E', 2), 'È': ('E', 3), 'Ẻ': ('E', 4), 'Ẽ': ('E', 5), 'Ẹ': ('E', 6),
            
            # Vowel 'i'
            'i': ('i', 1), 'í': ('i', 2), 'ì': ('i', 3), 'ỉ': ('i', 4), 'ĩ': ('i', 5), 'ị': ('i', 6),
            'I': ('I', 1), 'Í': ('I', 2), 'Ì': ('I', 3), 'Ỉ': ('I', 4), 'Ĩ': ('I', 5), 'Ị': ('I', 6),
            
            # Vowel 'o'
            'o': ('o', 1), 'ó': ('o', 2), 'ò': ('o', 3), 'ỏ': ('o', 4), 'õ': ('o', 5), 'ọ': ('o', 6),
            'O': ('O', 1), 'Ó': ('O', 2), 'Ò': ('O', 3), 'Ỏ': ('O', 4), 'Õ': ('O', 5), 'Ọ': ('O', 6),
            
            # Vowel 'u'
            'u': ('u', 1), 'ú': ('u', 2), 'ù': ('u', 3), 'ủ': ('u', 4), 'ũ': ('u', 5), 'ụ': ('u', 6),
            'U': ('U', 1), 'Ú': ('U', 2), 'Ù': ('U', 3), 'Ủ': ('U', 4), 'Ũ': ('U', 5), 'Ụ': ('U', 6),
            
            # Vowel 'y'
            'y': ('y', 1), 'ý': ('y', 2), 'ỳ': ('y', 3), 'ỷ': ('y', 4), 'ỹ': ('y', 5), 'ỵ': ('y', 6),
            'Y': ('Y', 1), 'Ý': ('Y', 2), 'Ỳ': ('Y', 3), 'Ỷ': ('Y', 4), 'Ỹ': ('Y', 5), 'Ỵ': ('Y', 6),
            
            # Compound vowels - ô
            'ô': ('ô', 1), 'ố': ('ô', 2), 'ồ': ('ô', 3), 'ổ': ('ô', 4), 'ỗ': ('ô', 5), 'ộ': ('ô', 6),
            'Ô': ('Ô', 1), 'Ố': ('Ô', 2), 'Ồ': ('Ô', 3), 'Ổ': ('Ô', 4), 'Ỗ': ('Ô', 5), 'Ộ': ('Ô', 6),
            
            # Compound vowels - ơ
            'ơ': ('ơ', 1), 'ớ': ('ơ', 2), 'ờ': ('ơ', 3), 'ở': ('ơ', 4), 'ỡ': ('ơ', 5), 'ợ': ('ơ', 6),
            'Ơ': ('Ơ', 1), 'Ớ': ('Ơ', 2), 'Ờ': ('Ơ', 3), 'Ở': ('Ơ', 4), 'Ỡ': ('Ơ', 5), 'Ợ': ('Ơ', 6),
            
            # Compound vowels - ư
            'ư': ('ư', 1), 'ứ': ('ư', 2), 'ừ': ('ư', 3), 'ử': ('ư', 4), 'ữ': ('ư', 5), 'ự': ('ư', 6),
            'Ư': ('Ư', 1), 'Ứ': ('Ư', 2), 'Ừ': ('Ư', 3), 'Ử': ('Ư', 4), 'Ữ': ('Ư', 5), 'Ự': ('Ư', 6),
        }
        ...
        self.common_family_names = {
            ...
            "Bùi", "Đỗ", "Hồ", "Ngô", "Dương", "Lý", "Lâm", "Lương", "Trương", "Đinh"
        }
        
        # Common Vietnamese titles to remove
        self.titles = {
            "Ông", "Bà", "Cô", "Chú", "Anh", "Chị", "Em",  # Family terms
            "Tiến sĩ", "Tiến sỹ", "Bác sĩ", "Giáo sư", "Thầy", "Cô",  # Professional
            "Dr", "Dr.", "Prof", "Prof.", "Mr", "Mr.", "Mrs", "Mrs.", "Ms", "Ms."  # English
        }
