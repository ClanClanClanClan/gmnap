...
class E7MaritimeSEA(RegionSpec):
    ...
    def __init__(self):
        ...
        self.indonesian_mononym_indicators = {
            # Common single names
            "Suharto", "Sukarno", "Megawati", "Jokowi", "Prabowo",
            "Wiranto", "Habibie", "Wahid", "Susilo", "Bambang",
            # Javanese single names
            "Soekarno", "Soeharto", "Abdurrahman", "Kalla",
            # Patterns that suggest mononyms
            "Harto", "Karno", "Wati", "Joko"
        }
        
        # Filipino naming patterns (Rule 30)
        # Common Filipino surnames and maternal indicators
        self.filipino_surnames = {
            "Santos", "Cruz", "Reyes", "Ramos", "Mendoza", "Garcia", "Rodriguez", 
            "Fernandez", "Lopez", "Gonzales", "Hernandez", "Perez", "Sanchez",
        ...
        self.titles = {
            # Malay/Indonesian
            "Dato", "Datuk", "Tan Sri", "Tun", "Haji", "Hajjah",
            "Encik", "Puan", "Cik", "Tuan", "Dr", "Prof",
            # Filipino  
            "Don", "Doña", "Ginoo", "Ginang", "Bb.", "G.",
            # English (common in region)
            "Mr", "Mr.", "Mrs", "Mrs.", "Ms", "Ms.", "Dr.", "Prof."
        }
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        if not components.get("is_mononym") and components.get("family_name") and components.get("given_name"):
    ...
    def _is_mononym(self, name: str) -> bool:
        ...
        if len(words) == 1:
            ...
            if (name_lower.endswith(('harto', 'karno', 'wati', 'joko')) or
                name_lower.startswith(('su', 'mega', 'pra', 'bam')) or
                len(words[0]) > 6):  # Long single names often mononyms
                return True
    ...
    def _normalize_mononym_canonical(self, name: str) -> str:
        ...
        suffixes_to_remove = {
            'jr', 'jr.', 'senior', 'senior.', 'ii', 'iii', 'iv',
            'muda', 'tua', 'besar', 'kecil'  # Indonesian generational markers
        }
    ...
    def _generate_no_patronymic_variant(self, name: str, patronymic_info: Dict[str, str]) -> Optional[str]:
