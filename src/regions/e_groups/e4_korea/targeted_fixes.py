...
def apply_targeted_weight_fixes(patterns):
    ...
    if 'jung_to_jeong' in patterns and len(patterns['jung_to_jeong']) > 0:
        fixes.append(('정', 'jeong', '-1.2', f"Fix {len(patterns['jung_to_jeong'])} jung→jeong cases"))
    ...
    if 'june_segmentation' in patterns and len(patterns['june_segmentation']) > 0:
        fixes.append(('준', 'june', '-1.0', f"Fix {len(patterns['june_segmentation'])} June segmentation cases"))
    ...
    for hangul, roman, new_weight, rationale in fixes:
        ...
        for i, row in enumerate(rows):
            if len(row) >= 3 and row[0] == hangul and row[1] == roman:
                old_weight = row[2]
                rows[i][2] = new_weight
                print(f"    Updated Line {i+1}: {hangul},{roman},{old_weight} → {hangul},{roman},{new_weight}")
                changes_made += 1
                found = True
                break
