...
class SystematicImprovementFrameworkV2:
    ...
    def _validate_weight(self, weight_str):
        ...
        if not self.WEIGHT_REGEX.match(weight_clean):
            raise ValueError(f"Invalid weight format: '{weight_str}' (must match {self.WEIGHT_REGEX.pattern})")
        
        weight_val = float(weight_clean)
        if not (self.WEIGHT_MIN <= weight_val <= self.WEIGHT_MAX):
            raise ValueError(f"Weight {weight_val} outside range [{self.WEIGHT_MIN}, {self.WEIGHT_MAX}]")
    ...
    def capture_baseline_performance(self):
        ...
        for dataset_name, test_script in datasets.items():
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    baseline["performance"][dataset_name] = {"error": f"Exit code {result.returncode}"}
                    continue
                ...
                if "success" in performance and "total" in performance:
                    ...
                    baseline["statistical_analysis"][dataset_name] = {
                        ...
                        "standard_error": ((performance["accuracy"] * (100 - performance["accuracy"])) / performance["total"]) ** 0.5
                    }
                
                print(f"  {dataset_name}: {performance['accuracy']:.2f}% ({performance['success']}/{performance['total']}) [Wilson LB: {wilson_lb:.2f}%]")
        ...
        baseline_file = self.audit_dir / f"baseline_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    ...
    def add_systematic_mappings(self, category, mappings, rationale=""):
        ...
        try:
            ...
            if validation_result["passed"]:
                ...
                self._log_improvement_v2(category, validated_mappings, rationale, baseline, validation_result)
    ...
    def _validate_all_datasets_v2(self, baseline):
        ...
        for dataset_name, test_script in datasets.items():
            try:
                ...
                if dataset_name in baseline["performance"] and "accuracy" in baseline["performance"][dataset_name]:
                    ...
                    baseline_se = baseline["statistical_analysis"].get(dataset_name, {}).get("standard_error", 1.0)
    ...
    def _log_improvement_v2(self, category, mappings, rationale, baseline, validation_result):
        ...
        log_file = self.audit_dir / f"improvement_log_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}.json"
    ...
    def _calculate_improvements_v2(self, baseline, validation_result):
        ...
        for dataset_name in baseline["performance"]:
            ...
            if ("error" not in baseline_perf and "error" not in final_perf and
                "accuracy" in baseline_perf and "accuracy" in final_perf):
                ...
                baseline_se = baseline.get("statistical_analysis", {}).get(dataset_name, {}).get("standard_error", 1.0)
