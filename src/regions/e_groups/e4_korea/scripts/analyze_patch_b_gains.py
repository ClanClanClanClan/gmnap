...
for name, info in data.items():
    if isinstance(info, dict) and 'CJK' in info:
        ...
        if actual:
            ...
            if roundtrip and roundtrip.replace(" ", "").lower() == romanized.replace(", ", "").replace(" ", "").lower():
                ...
                if any(pattern in name.lower() for pattern in ['cheong', 'jeong', 'jung', 'min', 'ho', 'jin']):
...
weighted_patterns = {
    'kim': 0, 'lee': 0, 'park': 0, 'jung': 0, 'jeong': 0,
    'min': 0, 'ho': 0, 'jin': 0, 'jun': 0, 'cheong': 0
}
