...
print(f"Success rate: {(total_cases - len(eng_to_kor_failures) - len(roundtrip_failures))/total_cases*100:.1f}%")
...
for f in roundtrip_failures[:10]:
    print(f"{f['name']}: {f['input']} → {f['korean']} → {f['got_romanization']} (dice: {f['dice_score']:.3f})")