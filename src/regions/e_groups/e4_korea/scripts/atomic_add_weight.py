...
def add_weight_to_csv(weight_line):
    ...
    with tempfile.NamedTemporaryFile(mode='w', encoding='utf-8', 
                                     dir=csv_path.parent, delete=False) as tmp:
...
def rebuild_fsts():
    """Rebuild FST models."""
    result = subprocess.run([sys.executable, "scripts/build_fsts_multi.py"], 
...
def run_regression_check():
    ...
    result = subprocess.run([sys.executable, "scripts/validate_regression.py"],
                          capture_output=True, text=True)
    if result.returncode != 0:
