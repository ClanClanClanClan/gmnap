...
class FullComprehensiveAudit:
    ...
    def audit_section_1_reproducibility(self):
        ...
        try:
            ...
            if valid_matches == len(valid_weights) and invalid_matches == 0:
                print(f"   ✓ Weight validation: {valid_matches}/{len(valid_weights)} valid, {invalid_matches}/{len(invalid_weights)} invalid rejected")
                section_results["passed"] += 1
                section_results["details"].append("Weight validation regex working")
            else:
                print(f"   ✗ Weight validation inconsistent: {valid_matches}/{len(valid_weights)} valid, {invalid_matches}/{len(invalid_weights)} invalid")
        ...
        try:
            ...
            required_fields = ["timestamp", "framework_version", "git_commit", "mapping_sha256_before"]
    ...
    def audit_section_4_data_processing(self):
        ...
        try:
            ...
            all_correct = all(bool(pattern.match(case)) == expected for case, expected in test_cases)
    ...
    def run_full_audit(self):
        ...
        print(f"Success rate: {self.audit_results['passed_checks']/self.audit_results['total_checks']*100:.1f}%")
