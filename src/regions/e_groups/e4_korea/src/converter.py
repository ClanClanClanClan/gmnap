...
def kor2eng(h: str, original_rr: str | None = None) -> str | None:
    ...
    if not outs:
        # Debug trace for missing outputs (disabled in production)
        # logger.debug(f"TRACE_NONE: {h}")
        return None
    if original_rr:
    ...
    # Fix capitalization: V7 spec requires "Kim Min-su" not "Kim Min-Su"
    # Rule: Family name capitalized, given name only first letter capitalized
    if result:
        parts = result.split()
        if len(parts) >= 2:
            # First part is family name (keep as-is)
            # Remaining parts are given name syllables
            fixed_parts = [parts[0]]
