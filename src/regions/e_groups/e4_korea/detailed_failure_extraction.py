        ...
        if orig_chars[i] == round_chars[j]:
            ...
            if not match_found:
                for k in range(i + 1, min(i + 4, len(orig_chars))):
                    if k < len(orig_chars) and j < len(round_chars) and orig_chars[k] == round_chars[j]:
            ...
            if not match_found:
                # Substitution
                differences.append(f"'{orig_chars[i]}' → '{round_chars[j] if j < len(round_chars) else 'END'}'")
                i += 1
                j += 1
    ...
    for pattern, cases in sorted(patterns.items(), key=lambda x: len(x[1]), reverse=True):
        ...
        for case in cases[:3]:
            if case['roundtrip_english']:
                print(f"  • {case['canonical_english']} → {case['roundtrip_english']} (dice: {case['dice_score']:.3f})")
            else:
