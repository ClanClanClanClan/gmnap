...
def identify_recalibration_targets():
    ...
    targets = [
        # 석 character mappings - need better weight distribution
        {
            ...
            "current_weight": "-0.4",
            "new_weight": "-0.8",
            "rationale": "Primary romanization, increase preference",
        },
        {
            "hangul": "석",
            "roman": "suk",
            "current_weight": "-0.223",
            "new_weight": "0.2",
            "rationale": "Secondary romanization, decrease preference",
        },
        {
            "hangul": "석",
            "roman": "sok",
            "current_weight": "0.0",
            "new_weight": "0.5",
            "rationale": "Tertiary romanization, further decrease",
        },
        # 숙 character mapping - very high cost needs adjustment
        {
            "hangul": "숙",
            "roman": "suk",
            "current_weight": "0.981",
            "new_weight": "0.3",
            "rationale": "Reduce excessive cost for common name element",
        },
    ]

    for target in targets:
        print(
            f"  {target['hangul']} → {target['roman']}: {target['current_weight']} → {target['new_weight']} ({target['rationale']})"
        )
        ...
        for i, row in enumerate(rows):
            if len(row) >= 3 and not row[0].startswith("#"):
                ...
                if hangul == target_hangul and roman == target_roman:
                    if dry_run:
                        print(
                            f"  Would change Line {i+1}: {hangul},{roman},{weight} → {hangul},{roman},{new_weight}"
                        )
                    else:
                        rows[i][2] = new_weight
                        print(
                            f"  Changed Line {i+1}: {hangul},{roman},{weight} → {hangul},{roman},{new_weight}"
                        )
                    changes_made += 1
