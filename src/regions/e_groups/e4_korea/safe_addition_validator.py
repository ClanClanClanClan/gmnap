...
class SafeAdditionValidator:
    ...
    def test_proposed_addition(self, proposed_weights=None, proposed_equivalences=None, description="Proposed change"):
    ...
    def find_safe_weight_for_case(self, failing_case_input, failing_case_expected, max_attempts=5):
        ...
        approaches = [
            ...
            (f"{failing_case_expected[0]},{failing_case_input.split(',')[0].strip().lower()},-1.5,SN,S", "Conservative surname weight"),
            (f"{failing_case_expected[-1]},{failing_case_input.split()[-1].lower()},-1.5,GN,G", "Conservative given weight"),
            
            # Medium strength weights
            (f"{failing_case_expected[0]},{failing_case_input.split(',')[0].strip().lower()},-2.5,SN,S", "Medium surname weight"),
            (f"{failing_case_expected[-1]},{failing_case_input.split()[-1].lower()},-2.5,GN,G", "Medium given weight"),
            
            # Strong weights (last resort)
            (f"{failing_case_expected[0]},{failing_case_input.split(',')[0].strip().lower()},-3.5,SN,S", "Strong surname weight"),
        ]
        ...
        return {
            'success': False,
            'reason': 'All approaches cause regressions'
        }
...
def example_find_safe_fix():
    ...
    result = validator.find_safe_weight_for_case(
        failing_case_input="Kim, David",
        failing_case_expected="김데이비드"
    )
