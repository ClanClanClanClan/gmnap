...
class E1_SinophoneMainland(RegionSpec):
    ...
    def _generate_pinyin(self, name: str) -> str:
        """Generate pinyin romanization using best available method."""

        # Try to use pypinyin library if available
        try:
            import pypinyin

            # Use pypinyin for accurate conversion
            pinyin_list = pypinyin.lazy_pinyin(name, style=pypinyin.Style.NORMAL)

            # Format based on name length
            if len(pinyin_list) == 3:  # Common pattern: 1 char family + 2 char given
                # Capitalize family name, capitalize first char of given name
                family = pinyin_list[0].capitalize()
                given = pinyin_list[1].capitalize() + pinyin_list[2].lower()
                return f"{family} {given}"
            elif len(pinyin_list) == 2:  # Pattern: 1 char family + 1 char given
                family = pinyin_list[0].capitalize()
                given = pinyin_list[1].capitalize()
                return f"{family} {given}"
            else:
        ...
        if canonical_native and canonical_latin:
            ...
            if dice_score < 0.97:
                raise RegionRuleError(
                    f"Rule 11 violation: CJK round-trip Dice coefficient {dice_score:.3f} < 0.97"
                )
