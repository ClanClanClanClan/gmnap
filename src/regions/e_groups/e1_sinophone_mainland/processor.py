...
class E1_SinophoneMainland(RegionSpec):
    ...
    def clean(self, entry: Dict[str, Any]) -> None:
        ...
        for field in ["CanonicalLatin", "CanonicalNative"]:
            if field in entry and entry[field]:
                ...
                try:
                    temp_entry = {field: raw_input, "GlobalID": "temp_validation"}
                    self.apply_security_and_validation_checks(temp_entry)
                except RegionRuleError:
                    if False:  # This line will be replaced
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        """Validate entry according to E1 rules using standardized validation."""
        # Apply comprehensive security and validation checks from base class
        self.apply_security_and_validation_checks(entry)

        # E1-specific validation: Chinese script preferences
        canonical_native = entry.get("CanonicalNative", "")
        canonical_latin = entry.get("CanonicalLatin", "")

        # If CanonicalNative exists, prefer it to be Chinese (but be lenient)
        if canonical_native and len(canonical_native.strip()) > 0:
            if not self._is_chinese(canonical_native):
                # Only warn for non-Chinese native names in E1 region
                self.logger.warning(
                    f"CanonicalNative in E1 region is not Chinese: {canonical_native}"
                )
                # Continue processing instead of failing

            # Warn about unusual length but don't fail
            if len(canonical_native) > 10:  # Very generous limit
                self.logger.warning(f"Unusually long Chinese name: {canonical_native}")
        ...
        if canonical_native and canonical_latin:
            try:
                ...
                # Don't fail validation if round-trip check fails
                self.logger.warning(f"CJK round-trip check failed: {str(e)}")
                # Continue processing
    ...
    def _validate_cjk_roundtrip(
        ...
        try:
            ...
            self.logger.debug(
                f"Rule 11 Round-trip: {canonical_native} → {expected_romanization} → {reconstructed_cjk} (Dice: {dice_score:.3f})"
            )
