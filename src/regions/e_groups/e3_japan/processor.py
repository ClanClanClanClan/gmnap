...
class E3_Japan(RegionSpec):
    ...
    def clean(self, entry: Dict[str, Any]) -> None:
        ...
        for field in ["CanonicalLatin", "CanonicalNative"]:
            if field in entry and entry[field]:
                ...
                # Normalize tabs and newlines to spaces (V7 edge case)
                if "\t" in raw_input:
                ...
                if "\n" in raw_input:
                    raw_input = raw_input.replace("\n", " ")
                    entry[field] = raw_input
                # Check for remaining dangerous control characters
        # V7 Security: Use comprehensive security validation from base class
        self.apply_security_and_validation_checks(entry)
    ...
    def augment(self, entry: Dict[str, Any]) -> None:
        ...
        if self._is_japanese(canonical):
            ...
            if romanized != canonical:
                # Set CanonicalLatin if not already romanized
                canonical_latin = entry.get("CanonicalLatin", "")
                if not canonical_latin or self._is_japanese(canonical_latin):
                    entry["CanonicalLatin"] = romanized
                # Also add as synthesized variant
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        # This properly handles empty fields with graceful degradation
        self.apply_security_and_validation_checks(entry)

        # E3-specific validation
        canonical_native = entry.get("CanonicalNative", "")
        canonical_latin = entry.get("CanonicalLatin", "")

        # If CanonicalNative exists, it should be Japanese - but be lenient for edge cases
        if canonical_native:
    ...
    def _is_valid_romanization(self, text: str) -> bool:
        ...
        for word in words:
            # Should only contain Latin letters, basic marks, and spaces
            if word and not re.match(r"^[a-zA-Z][a-zA-Z-]*$", word):
                return False
        return len(words) >= 1  # At least one word required
