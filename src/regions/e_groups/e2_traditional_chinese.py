...
class E2TraditionalChineseProcessor(RegionSpec):
    ...
    def validate(self, entry: Dict[str, Any]) -> None:
        ...
        if not self._is_valid_traditional_chinese_name(name_to_check):
            invalid_chars = set(name_to_check) - set("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 ,.-'") - self.allowed_chars
        ...
        if script == "Chinese" and not any(self._is_chinese_char(c) for c in name_to_check):
            raise RegionRuleError("Name marked as Chinese script but contains no Chinese characters")
        ...
        if components.get("has_generation_name"):
            ...
            if generation_char not in given:
                raise RegionRuleError(f"Generation character {generation_char} not found in given name: {given}")
