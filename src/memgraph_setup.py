...
class MemgraphManager:
    ...
    def __init__(self, bolt_uri: str = "bolt://localhost:7687", auth: tuple = ("", "")):
    ...
    def compute_graph_coherence_score(self) -> float:
        ...
        with self.driver.session() as session:
            ...
            if centralities:
                ...
                n = len(centralities)
                index = range(1, n + 1)
                gini = (2 * sum(index[i] * centralities[i] for i in range(n))) / (
                    n * sum(centralities)
                ) - (n + 1) / n
...
def setup_memgraph_docker():
    ...
    container = client.containers.run(
        ...
        volumes={"gmnap-memgraph-data": {"bind": "/var/lib/memgraph", "mode": "rw"}},
        environment={"MEMGRAPH_USER": "gmnap", "MEMGRAPH_PASSWORD": "gmnap123"},
        detach=True,
        remove=False,
    )
