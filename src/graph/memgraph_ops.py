...
class MemgraphPool:
    ...
    async def start(self):
        if AsyncGraphDatabase:
            self._driver = AsyncGraphDatabase.driver(self.uri, auth=self.auth, max_connection_pool_size=self.pool_size)
        return self
    ...
    async def session(self):
        ...
        if self._driver:
            ...
            class _S:  # stub
                async def run(self, *a, **k): return type("R", (), {"data": lambda self: []})()
            try:
                yield _S()
            finally:
                self._sem.release()
...
async def ensure_indices(pool: MemgraphPool):
    async with pool.session() as s:
        if getattr(s, "run", None):
            await s.run("CREATE INDEX mathematician_gid IF NOT EXISTS FOR (m:Mathematician) ON (m.global_id)")
...
async def write_entries_and_edges(pool: MemgraphPool, entries: List[Dict]) -> int:
    ...
    async with pool.session() as s:
        for e in entries:
            ...
            if getattr(s, "run", None):
                await s.run("MERGE (m:Mathematician {global_id:$gid}) SET m.canonical=$name", gid=gid, name=name)
