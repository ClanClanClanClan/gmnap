...
async def enrich_all(entries: List[Dict]) -> List[Dict]:
    ...
    for e in entries:
        ...
        merged["Sources"] = sorted(list({*(merged.get("Sources") or []), *[k for k,_ in AUTH_HANDLERS]}))
        # Merge advisor edges
        for (name, _), r in zip(AUTH_HANDLERS, results):
            if isinstance(r, dict) and name == "Wikidata_P184":
                edges = r.get("Wikidata_P184") or []
                if edges:
                    merged["Advisors"] = sorted(list({*(merged.get("Advisors") or []), *[edge["target"] for edge in edges]}))
        out.append(merged)
    return out
