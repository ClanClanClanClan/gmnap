...
class E4_Korea(BaseRegionHandler):
    ...
    def __init__(self):
        ...
        # Initialize V5 Korean processing if available
        if V5_AVAILABLE:
            self.korean_converter = KoreanConverter()
            self.hangul_converter = HangulToRomanConverter("rr")
            self.segmenter = KoreanSegmenter()
            self.logger.info("V5 Korean processing initialized")
        else:
            self.korean_converter = None
            self.hangul_converter = None
            self.segmenter = None
            self.logger.error("V5 Korean processing not available")
    ...
    def augment(self, entry: Dict) -> Dict:
        ...
        """
        if not V5_AVAILABLE:
            self.logger.warning("Cannot augment without V5 Korean processing")
            return entry
    ...
    def validate(self, entry: Dict) -> Tuple[bool, List[str]]:
        ...
        # Validate round-trip accuracy if V5 available
        if V5_AVAILABLE and entry.get("CanonicalLatin"):
    ...
    def generate_order_key(self, entry: Dict) -> str:
        """
...
if __name__ == "__main__":
    ...
    order_key = handler.generate_order_key(augmented)
    print(f"\nOrder key: {order_key}")