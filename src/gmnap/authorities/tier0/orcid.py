...
class ORCIDFetcher(AuthorityFetcher):
    ...
    def _is_orcid_id(self, query: str) -> bool:
        ...
        return (len(query) == 19 and 
                query.count('-') == 3 and
                all(c.isdigit() or c == '-' for c in query))
        ...
        async with session.get(url, headers=headers) as response:
            if response.status == 200:
                ...
                return FetchResult(
                    ...
                    error_message="ORCID ID not found"
                )
    ...
    def _find_best_match(self, results: List[Dict[str, Any]], query: str) -> Optional[Dict[str, Any]]:
