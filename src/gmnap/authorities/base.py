...
class AuthorityFetcher(ABC):
    ...
    async def get_session(self) -> aiohttp.ClientSession:
        ...
        if self._session is None or self._session.closed:
            ...
            headers = {
                'User-Agent': self.config.get('user_agent', 'GMNAP/1.0')
            }
...
class QuotaManager:
    ...
    def __init__(self, source_manifest: Dict[str, Any], cache_dir: Optional[Union[str, Path]] = None):
    ...
    async def acquire_quota(self, service: str, count: int = 1) -> bool:
        ...
        async with self._lock:
            ...
            if used + count > quota:
                logger.warning(
                    f"Quota exceeded for {service}: {used + count}/{quota}"
                )
                return False
