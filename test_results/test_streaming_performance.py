...
def test_pipeline_performance(pipeline_class, dataset_size: int, config: GMNAPConfig, 
                            streaming_config=None) -> dict:
...
def run_performance_comparison():
    ...
    for size in test_sizes:
        ...
        try:
            ...
            if regular_result["success"]:
                print(f"  ✅ Success: {regular_result['duration_seconds']:.1f}s, "
        ...
        try:
            ...
            if streaming_result["success"]:
                print(f"  ✅ Success: {streaming_result['duration_seconds']:.1f}s, "
        ...
        if regular_result.get("success") and streaming_result.get("success"):
            speed_improvement = streaming_result["entries_per_second"] / regular_result["entries_per_second"]
            memory_improvement = regular_result["memory_increase_mb"] / max(streaming_result["memory_increase_mb"], 1)
    ...
    for size in test_sizes:
        ...
        if regular_key in results and streaming_key in results:
            ...
            if regular.get("success"):
                print(f"  Regular:   {regular['duration_seconds']:.1f}s, "
            ...
            if streaming.get("success"):
                print(f"  Streaming: {streaming['duration_seconds']:.1f}s, "
    ...
    for size in test_sizes:
        ...
        if key in results and results[key].get("success"):
            if not best_streaming or results[key]["entries_per_second"] > best_streaming["entries_per_second"]:
    ...
    if best_streaming:
        ...
        if projected_time_minutes <= 30:
            ...
            print(f"  ⚠️  Still {projected_time_minutes/30:.1f}x slower than 30min target")
    else:
        print(f"  ❌ No successful streaming runs to extrapolate from")
