...
class Pipeline:
    ...
    async def execute(self, initial_input: Any = None) -> Dict[str, Any]:
        ...
        return {
            "status": "completed" if all(r.status == StageStatus.COMPLETED for r in results) else "partial",
    ...
    def _update_metrics(self, result: StageResult) -> None:
        ...
        self._metrics["total_records_processed"] = self._metrics.get("total_records_processed", 0) + result.records_processed
        self._metrics["total_records_failed"] = self._metrics.get("total_records_failed", 0) + result.records_failed
...
class ValidationStage(PipelineStage[List[Dict[str, Any]], List[Dict[str, Any]]]):
    ...
    async def process(self, input_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for record in input_data:
            try:
                if self.validator.validate_entry(record):
