...
class GlobalIDGenerator:
    ...
    def load_existing_ids(self, ids: Set[str]) -> None:
        ...
        logger.info(f"Loaded {len(ids)} existing IDs (deterministic generator doesn't need pre-loading)")
        pass
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        base_ids_with_collisions = sum(1 for entries in self._true_collisions.values() if len(entries) > 1)
        total_collisions = sum(sum(collision_nums for collision_nums in entries.values() if collision_nums > 0) 
