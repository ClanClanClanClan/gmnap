...
class StreamingPipeline(GMNAPPipeline):
    ...
    def __init__(self, config: GMNAPConfig, mode: PipelineMode, streaming_config: Optional[StreamingConfig] = None):
    ...
    def _create_streaming_tables(self) -> None:
        ...
        self._temp_db.execute("CREATE INDEX IF NOT EXISTS idx_entries_canonical ON entries(canonical_latin)")
        self._temp_db.execute("CREATE INDEX IF NOT EXISTS idx_entries_chunk ON entries(chunk_id)")
        self._temp_db.execute("CREATE INDEX IF NOT EXISTS idx_entries_stage ON entries(processing_stage)")
    ...
    def run(self, input_path: Path) -> Any:
        ...
        try:
            ...
            logger.info(f"Processing {self._total_chunks} chunks with {self.streaming_config.chunk_size} entries each")
    ...
    def _cleanup_memory(self) -> None:
        ...
        try:
            ...
            if memory_mb > self.streaming_config.memory_limit_mb:
                logger.warning(f"Memory usage ({memory_mb:.1f} MB) exceeds limit ({self.streaming_config.memory_limit_mb} MB)")
