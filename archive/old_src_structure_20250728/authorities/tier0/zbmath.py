...
from src.authorities.base import (AuthorityData, AuthorityFetcher,
                                  AuthorityTier, FetchResult, FetchStatus)
...
class ZbMATHFetcher(AuthorityFetcher):
    ...
    def parse_search_results(self, documents: List[Dict[str, Any]], query: str) -> AuthorityData:
        ...
        if author_stats["name_variants"]:
            ...
            canonical_name = max(author_stats["name_variants"], 
        ...
        # Add MSC codes (top 5)
        msc_codes = []
        for code, count in sorted(author_stats["msc_codes"].items(), 
        ...
        data.metadata = {
            ...
            "year_range": f"{min(author_stats['years'])}-{max(author_stats['years'])}" if author_stats["years"] else "",
    ...
    def calculate_confidence(self, data: AuthorityData, stats: Optional[Dict[str, Any]] = None) -> float:
        """
